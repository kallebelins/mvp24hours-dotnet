...
O Audit Trail registra todas as operações de escrita (commands) para fins de auditoria, compliance e debugging.
    ...
    where TRequest : IMediatorCommand<TResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Capturar EntityId do resultado se disponível
...
1. **Separar Database**: Use banco separado para auditoria
2. **Retenção**: Configure política de retenção
3. **Imutabilidade**: Audit logs são append-only
4. **Compressão**: Comprima dados antigos
5. **Índices**: Otimize queries comuns
6. **GDPR**: Considere anonimização de dados
7. **Performance**: Use async e batch writes
