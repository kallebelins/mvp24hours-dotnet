...
Este guia mostra como implementar sagas completas usando o padrão de orquestração.

## Estrutura de Dados da Saga
...
1. **Ordem Clara**: Use `Order` para definir sequência
2. **Idempotência**: Steps devem ser idempotentes
3. **Logging**: Log cada passo e compensação
4. **Estado Persistido**: Salve estado para recuperação
5. **Timeout**: Configure timeout para cada step
6. **Retry**: Implemente retry para falhas transientes
