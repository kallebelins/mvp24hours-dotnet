# Padrões de Versionamento de API para Agentes de IA

> **Instrução para Agente de IA**: Use estes padrões ao implementar versionamento de API em aplicações baseadas em Mvp24Hours. Escolha a estratégia de versionamento apropriada com base nos requisitos do projeto.

---

## Estratégias de Versionamento

| Estratégia | Exemplo | Caso de Uso |
|------------|---------|-------------|
| URL Path | `/api/v1/customers` | APIs RESTful, separação clara |
| Query String | `/api/customers?api-version=1.0` | Retrocompatibilidade |
| Header | `X-API-Version: 1.0` | URLs limpas |
| Media Type | `Accept: application/vnd.api.v1+json` | Negociação de conteúdo |
...
## Versionamento por URL Path (Recomendado)
...
public class CustomersController : ControllerBase
{
    ...
    public async Task<IActionResult> GetById(int id)
    {
        // V2 retorna resposta aprimorada com metadados
...
public class CustomersController : ControllerBase
{
    ...
    {
        // Implementação V2 com resposta aprimorada
    }
}
```
...
## Versionamento Combinado (Múltiplos Readers)
...
public static IServiceCollection AddCombinedVersioning(this IServiceCollection services)
{
    services.AddApiVersioning(options =>
    {
        ...
        // Permitir versão especificada de múltiplas formas
...
## Configuração do Swagger para APIs Versionadas
...
public class ConfigureSwaggerOptions : IConfigureOptions<SwaggerGenOptions>
{
    ...
    private static OpenApiInfo CreateInfoForApiVersion(ApiVersionDescription description)
    {
        var info = new OpenApiInfo
        {
            ...
            Description = "Documentação da API para ProjectName",
            Contact = new OpenApiContact
            {
                Name = "Suporte",
                Email = "suporte@projectname.com"
            }
        };
...
## Descontinuação de Versão

### Marcando Versões como Descontinuadas

```csharp
// Controllers/V1/CustomersController.cs
[ApiController]
[ApiVersion("1.0", Deprecated = true)]  // Marcar como descontinuada
[Route("api/v{version:apiVersion}/[controller]")]
public class CustomersController : ControllerBase
{
    // Endpoints V1 (descontinuados)
}
```
...
public static class CustomerVersionMapper
{
    public static V2.CustomerDto ToV2(this V1.CustomerDto v1, CustomerMetadata metadata)
    {
        return new V2.CustomerDto(
            ...
            v1.Name,
            v1.Email,
            DateTimeOffset.UtcNow, // Padrão para upgrade
            null,
            metadata
        );
    }
