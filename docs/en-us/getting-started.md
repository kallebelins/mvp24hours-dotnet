...
Build every sample at once with [`samples/Mvp24Hours.Samples.slnx`](https://github.com/kallebelins/mvp24hours-dotnet/blob/main/samples/Mvp24Hours.Samples.slnx). With Mvp24Hours **10.9.0** published, switch to NuGet mode with `-p:Mvp24HoursUseProjectReferences=false` — see [Using samples with published NuGet packages](migration.md?id=using-samples-with-published-nuget-packages).
