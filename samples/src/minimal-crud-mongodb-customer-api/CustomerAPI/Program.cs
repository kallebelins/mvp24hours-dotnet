...
app.MapGet("/customer/{id}", async (string id, [FromServices] IUnitOfWorkAsync uoW, CancellationToken cancellationToken) =>
{
...
app.MapPut("/customer/{id}", async (string id, Customer model, [FromServices] IUnitOfWorkAsync uoW, [FromServices] IValidator<Customer> validator, CancellationToken cancellationToken) =>
{
    // apply data validation to the model/entity with FluentValidation or DataAnnotation
    var errors = model.TryValidate(validator);
...
app.MapDelete("/customer/{id}", async (string id, [FromServices] IUnitOfWorkAsync uoW, CancellationToken cancellationToken) =>
{
