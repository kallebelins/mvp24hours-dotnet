    ...
    ILogger<ContactService> logger) : RepositoryPagingServiceAsync<Contact, IUnitOfWorkAsync>(unitOfWork, validator), IContactService
{
    ...
    public async Task<IBusinessResult<IList<ContactIdResult>>> GetBy(int customerId, CancellationToken cancellationToken = default)
    {
        ...
        IEnumerable<Contact> result = await UnitOfWork
            .GetConnection()
        ...
        return _mapper.Map<IList<ContactIdResult>>(result).ToBusiness();
    }
    ...
    public async Task<IBusinessResult<int>> Create(int customerId, ContactCreate dto, CancellationToken cancellationToken = default)
    {
        ...
        entity.Created = _timeProvider.GetUtcNow().UtcDateTime;
        entity.Active = true;
        ...
        {
            _logger.LogInformation("Created contact {ContactId} for customer {CustomerId}", entity.Id, customerId);
    ...
    {
        Contact? entity = await Repository.GetByAsync(x => x.Id == id && x.CustomerId == customerId, cancellationToken: cancellationToken).FirstOrDefaultAsync();
        if (entity == null)
        {
        ...
        if (affectedRows > 0)
        {
            _logger.LogInformation("Updated contact {ContactId} for customer {CustomerId}", id, customerId);
    ...
    public async Task<IBusinessResult<int>> Delete(int customerId, int id, CancellationToken cancellationToken = default)
    {
        ...
        if (affectedRows > 0)
        {
            _logger.LogInformation("Deleted contact {ContactId} for customer {CustomerId}", id, customerId);
            return affectedRows.ToBusiness(
                Messages.OPERATION_SUCCESS
                    .ToMessageResult(nameof(Messages.OPERATION_SUCCESS), MessageType.Success));
        }
