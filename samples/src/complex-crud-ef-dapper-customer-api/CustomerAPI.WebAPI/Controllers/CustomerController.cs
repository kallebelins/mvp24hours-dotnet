...
/// <summary>
/// Customer HTTP resources with DTO contracts and Dapper-backed reads.
/// </summary>
...
public class CustomerController(FacadeService facade) : ControllerBase
{
    ...
    /// </summary>
    [HttpGet]
    [ProducesResponseType(typeof(ActionResult<IPagingResult<IEnumerable<CustomerResult>>>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ActionResult<IPagingResult<IEnumerable<CustomerResult>>>), StatusCodes.Status404NotFound)]
    [Route("", Name = "CustomerGetBy")]
    public async Task<ActionResult<IPagingResult<IEnumerable<CustomerResult>>>> GetBy([FromQuery] CustomerQuery filter, [FromQuery] PagingCriteriaRequest pagingCriteria, CancellationToken cancellationToken)
    {
        IPagingResult<IEnumerable<CustomerResult>> result = await facade.CustomerService.GetBy(filter, pagingCriteria.ToPagingCriteria(), cancellationToken: cancellationToken);
        if (result.HasData())
        {
            return Ok(result);
        }
    ...
    {
        IBusinessResult<CustomerIdResult> result = await facade.CustomerService.GetById(id, cancellationToken: cancellationToken);
        if (result.HasData())
        {
            return Ok(result);
        }
    ...
    {
        IBusinessResult<int> result = await facade.CustomerService.Create(model, cancellationToken: cancellationToken);
        if (result.HasErrors)
        {
    ...
    {
        IBusinessResult<int> result = await facade.CustomerService.Update(id, model, cancellationToken: cancellationToken);
        if (result.HasErrors)
        {
    ...
    {
        IBusinessResult<int> result = await facade.CustomerService.Delete(id, cancellationToken: cancellationToken);
        if (result.HasErrors)
        {
            if (result.HasMessageKey(nameof(Messages.RECORD_NOT_FOUND_FOR_ID)))
            {
