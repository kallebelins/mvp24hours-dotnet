...
public abstract class AdvancedCronJobService<T> : ResilientCronJobService<T>
{
    ...
    public sealed override async Task DoWork(CancellationToken cancellationToken)
    {
        ...
        {
            bool satisfied = await _dependencyTracker.AreDependenciesSatisfiedAsync(JobName, cancellationToken);
            if (!satisfied)
            {
        ...
        {
            string instanceId = Environment.MachineName + "_" + Environment.ProcessId;
