...
public class RabbitMQConnection
{
    ...
    {
        string scheme = Ssl?.Enabled == true ? "amqps" : "amqp";
        string host = HostName ?? "localhost";
        int port = Port > 0 ? Port : (Ssl?.Enabled == true ? 5671 : 5672);
        string vhost = Uri.EscapeDataString(VirtualHost ?? "/");
        string user = Uri.EscapeDataString(UserName ?? "guest");
        string pass = Uri.EscapeDataString(Password ?? "guest");
