...
public class RetryPolicyBuilder
{
    ...
    public RetryPolicyBuilder Intervals(params TimeSpan[] intervals)
    {
        ...
        _configuration.Intervals = [.. intervals];
        return this;
    }
...
public class RetryPolicyConfiguration
{
    ...
    public TimeSpan GetDelay(int attempt)
    {
        ...
        {
            double jitterRange = baseDelay.TotalMilliseconds * MaxJitterPercent / 100.0;
            double jitter = (Random.Shared.NextDouble() - 0.5) * 2 * jitterRange;
            double adjustedMs = baseDelay.TotalMilliseconds + jitter;
            if (adjustedMs < 0)
            {
                adjustedMs = 0;
            }
