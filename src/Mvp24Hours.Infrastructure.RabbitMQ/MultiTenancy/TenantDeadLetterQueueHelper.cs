...
public class TenantDeadLetterQueueHelper(
    IOptions<TenantRabbitMQOptions> options,
    ITenantConnectionFactory connectionFactory,
    ILogger<TenantDeadLetterQueueHelper>? logger = null) : ITenantDeadLetterQueueHelper
{
    private readonly TenantRabbitMQOptions _options = options?.Value ?? throw new ArgumentNullException(nameof(options));
    private readonly ITenantConnectionFactory _connectionFactory = connectionFactory ?? throw new ArgumentNullException(nameof(connectionFactory));
    private readonly ILogger<TenantDeadLetterQueueHelper>? _logger = logger;
    ...
    public void EnsureDeadLetterInfrastructure(string tenantId)
    {
        ...
        try
        {
            ...
            string dlxName = GetDeadLetterExchangeName(tenantId);
            string dlqName = GetDeadLetterQueueName(tenantId);
