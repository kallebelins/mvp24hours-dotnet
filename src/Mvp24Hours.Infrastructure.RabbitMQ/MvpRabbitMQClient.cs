...
public class MvpRabbitMQClient : IMvpRabbitMQClient
{
    ...
    public MvpRabbitMQClient(IServiceProvider _provider)
    {
        ...
        _options = _provider.GetService<IOptions<RabbitMQClientOptions>>()?.Value
            ?? _provider.GetService<RabbitMQClientOptions>()
            ?? throw new ArgumentNullException(nameof(_provider));
        _connection = _provider.GetService<IMvpRabbitMQConnection>()
            ?? throw new ArgumentNullException(nameof(_provider));
        _channels = [];
        _consumers = [];
        _logger = _provider.GetService<ILogger<MvpRabbitMQClient>>();

        // Optional services
        if (_options.EnableMetrics)
        {
            _metrics = _provider.GetService<IRabbitMQMetrics>();
        }
        if (_options.EnableStructuredLogging)
        {
            _structuredLogger = _provider.GetService<IRabbitMQStructuredLogger>();
        }
        if (_options.Deduplication.Enabled)
        {
            _deduplicationStore = _provider.GetService<IMessageDeduplicationStore>();
        }
    }
    #endregion
    ...
    public virtual IEnumerable<string> PublishBatch(IEnumerable<(object Message, string RoutingKey)> messages)
    {
        ...
        try
        {
            ...
            {
                string tokenDefault = Guid.NewGuid().ToString();
                messageIds.Add(tokenDefault);

                IBusinessEvent bsEvent = msg.ToBusinessEvent(tokenDefault: tokenDefault);
                byte[] body = Encoding.UTF8.GetBytes(bsEvent.ToSerialize(JsonHelper.JsonBusinessEventSettings()));
        ...
        int? ttlMilliseconds)
    {
        ...
        try
        {
            ...
            RetryPolicy policy = RetryPolicy.Handle<BrokerUnreachableException>()
                .Or<SocketException>()
                .WaitAndRetry(Connection.Options.RetryCount, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)), (ex, time) => _logger?.LogWarning(ex,
    ...
    private IModel CreateConsumerChannel(Action<IModel, RabbitMQClientOptions, string, string> action, string? routingKey, string? queueName)
    {
        ...
        if (channel == null || channel.IsClosed)
        {
            ...
            string resolvedRoutingKey = routingKey ?? string.Empty;
    ...
    {
        if (channelCtx == null || optionsCtx == null)
        {
            return;
        }
    ...
    private void HandleConsume(BasicDeliverEventArgs e, IModel channel, IMvpRabbitMQConsumerSync consumerSync)
    {
        ...
        try
        {
            ...
            string messageId = properties.MessageId ?? properties.CorrelationId ?? Guid.NewGuid().ToString();

            // Deduplication check
            if (_deduplicationStore != null && Options.Deduplication.Enabled)
            {
                bool isProcessed = _deduplicationStore.IsProcessedAsync(messageId).GetAwaiter().GetResult();
                if (isProcessed)
                {
    ...
    private async Task HandleConsumeAsync(BasicDeliverEventArgs e, IModel channel, IMvpRabbitMQConsumerAsync consumerAsync)
    {
        ...
        try
        {
            ...
            if (_deduplicationStore != null && Options.Deduplication.Enabled)
            {
                bool isProcessed = await _deduplicationStore.IsProcessedAsync(messageId);
                if (isProcessed)
                {
                    _metrics?.IncrementDuplicateMessagesSkipped();
                    _structuredLogger?.LogDuplicateMessageSkipped(messageId);
