...
/// </remarks>
public class RateLimitingPublishFilter(
    IRateLimiterProvider rateLimiterProvider,
    RateLimitingPublishFilterOptions? options = null,
    ILogger<RateLimitingPublishFilter>? logger = null) : IPublishFilter
{
    private readonly IRateLimiterProvider _rateLimiterProvider = rateLimiterProvider ?? throw new ArgumentNullException(nameof(rateLimiterProvider));
    private readonly RateLimitingPublishFilterOptions _options = options ?? RateLimitingPublishFilterOptions.Default;
    private readonly ILogger<RateLimitingPublishFilter>? _logger = logger;
    ...
    {
        string key = GetRateLimiterKey(context);
        ...
        if (!lease.IsAcquired)
        {
            ...
            if (_options.WaitWhenExceeded && retryAfter.HasValue)
            {
                ...
                // Retry acquire
                using RateLimitLease retryLease = await _rateLimiterProvider.AcquireAsync(
                    key,
                    options,
                    1,
                    cancellationToken);
