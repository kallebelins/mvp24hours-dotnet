...
public class InMemoryBus(IServiceProvider serviceProvider) : IInMemoryBus
{
    ...
    /// <inheritdoc />
    public IReadOnlyList<IPublishedMessage> PublishedMessages => [.. _publishedMessages];

    /// <inheritdoc />
    public IReadOnlyList<IConsumedMessage> ConsumedMessages => [.. _consumedMessages];
    ...
    public IEnumerable<string> PublishBatch(IEnumerable<(object Message, string RoutingKey)> messages)
    {
        ...
        {
            string messageId = PublishInternal(message, routingKey, null, null, null, null, isBatch: true);
    ...
    private string PublishInternal(
        ...
        bool isBatch = false)
    {
        string messageId = tokenDefault ?? Guid.NewGuid().ToString();
