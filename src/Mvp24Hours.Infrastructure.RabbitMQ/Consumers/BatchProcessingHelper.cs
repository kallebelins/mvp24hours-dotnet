...
public static class BatchProcessingHelper
{
        ...
        where TMessage : class
    {
        ...
        IEnumerable<Task> tasks = context.Messages.Select(async item =>
        {
            ...
            try
            {
                bool success = await processFunc(item, cancellationToken);
                results.Add(success
        ...
        where TMessage : class
    {
        try
        {
            bool success = await processFunc(context, cancellationToken);
            return success
        ...
        where TKey : notnull
    {
        ...
        foreach (IGrouping<TKey, IBatchMessageItem<TMessage>>? group in groups)
        {
            ...
            try
            {
                ...
                bool success = await processGroupFunc(group.Key, groupItems, cancellationToken);
