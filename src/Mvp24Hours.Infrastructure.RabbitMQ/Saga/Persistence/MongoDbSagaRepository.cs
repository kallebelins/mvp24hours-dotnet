...
/// </remarks>
public class MongoDbSagaRepository<TData>(
    IMongoSagaCollection<TData> collection,
    ILogger<MongoDbSagaRepository<TData>>? logger = null) : ISagaRepository<TData> where TData : class, new()
{
    private readonly IMongoSagaCollection<TData> _collection = collection ?? throw new ArgumentNullException(nameof(collection));
    private readonly ILogger<MongoDbSagaRepository<TData>>? _logger = logger;
    ...
    {
        bool deleted = await _collection.DeleteAsync(correlationId, cancellationToken);
        if (deleted)
        {
    ...
    public async Task<int> CleanupAsync(TimeSpan olderThan, CancellationToken cancellationToken = default)
    {
        ...
        int cleaned = await _collection.DeleteCompletedOlderThanAsync(threshold, cancellationToken);
