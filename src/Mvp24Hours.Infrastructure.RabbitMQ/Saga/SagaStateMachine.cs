...
public abstract class SagaStateMachine<TData> where TData : class, new()
{
    ...
    /// </summary>
    public class SagaState(string name)
    {
...
public class SagaEventContext<TData, TEvent>(
    ...
    where TEvent : class
{
    private readonly object _consumeContext = consumeContext;

    /// <summary>
    /// Gets the saga instance.
    /// </summary>
    public SagaInstance<TData> Instance { get; } = instance;
    ...
    {
        return Instance.Metadata.TryGetValue(key, out string? value) ? value : null;
    }
