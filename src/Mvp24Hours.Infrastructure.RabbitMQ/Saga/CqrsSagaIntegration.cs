    ...
    where TSaga : SagaBase<TData>
{
    ...
    private readonly ISagaOrchestrator _orchestrator = orchestrator ?? throw new ArgumentNullException(nameof(orchestrator));
    private readonly ILogger<CqrsSagaAdapter<TData, TSaga>>? _logger = logger;
...
/// </remarks>
public class CqrsSagaMessageConsumer<TData, TSaga, TMessage>(
    CqrsSagaAdapter<TData, TSaga> adapter,
    Func<TMessage, TData> dataFactory,
    Func<TMessage, Guid>? correlationIdExtractor = null,
    ILogger<CqrsSagaMessageConsumer<TData, TSaga, TMessage>>? logger = null) : IMessageConsumer<TMessage>
    where TData : class
    where TSaga : SagaBase<TData>
    where TMessage : class
{
    private readonly CqrsSagaAdapter<TData, TSaga> _adapter = adapter ?? throw new ArgumentNullException(nameof(adapter));
    private readonly Func<TMessage, TData> _dataFactory = dataFactory ?? throw new ArgumentNullException(nameof(dataFactory));
    private readonly Func<TMessage, Guid>? _correlationIdExtractor = correlationIdExtractor;
    private readonly ILogger<CqrsSagaMessageConsumer<TData, TSaga, TMessage>>? _logger = logger;
