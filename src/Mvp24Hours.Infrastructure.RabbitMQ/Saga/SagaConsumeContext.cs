...
/// </remarks>
public class SagaConsumeContext<TData, TMessage>(
    ...
    ISagaRepository<TData>? repository = null) : ISagaConsumeContext<TData, TMessage>
    ...
    where TMessage : class
{
    ...
    private readonly SagaInstance<TData> _instance = instance ?? throw new ArgumentNullException(nameof(instance));
    private readonly IMessageScheduler? _scheduler = scheduler;
    private readonly IMvpRabbitMQClient? _rabbitMQClient = rabbitMQClient;
    private readonly ISagaRepository<TData>? _repository = repository;
        ...
        CancellationToken cancellationToken = default) where TTimeout : class
    {
        ...
        string routingKey = $"saga.{typeof(TData).Name}.timeout";
    ...
    public async Task<bool> CancelTimeoutAsync(Guid timeoutId, CancellationToken cancellationToken = default)
    {
        ...
        bool cancelled = await _scheduler.CancelScheduledMessageAsync(timeoutId, cancellationToken);
        if (cancelled)
        {
    ...
    {
        return _instance.Metadata.TryGetValue(key, out string? value) ? value : null;
    }
    ...
    /// </summary>
    public SagaInstance<TData> GetSagaInstance()
    {
        return _instance;
    }
}
