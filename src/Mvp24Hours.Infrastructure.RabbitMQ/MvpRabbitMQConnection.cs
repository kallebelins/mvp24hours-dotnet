...
public sealed class MvpRabbitMQConnection : IMvpRabbitMQConnection, IDisposable
{
    ...
    public RabbitMQConnectionOptions Options { get; }
    ...
    public bool TryConnect()
    {
        ...
        lock (sync_root)
        {
            RetryPolicy policy = RetryPolicy.Handle<SocketException>()
                ...
                .WaitAndRetry(Options.RetryCount, retryAttempt => TimeSpan.FromSeconds(Math.Pow(2, retryAttempt)), (ex, time) => _logger?.LogWarning(ex,
