...
public class EndpointNameFormatter(string prefix, EndpointNamingConventionOptions options) : IEndpointNameFormatter
{
    private readonly string _prefix = prefix ?? string.Empty;
    private readonly EndpointNamingConventionOptions _options = options ?? new EndpointNamingConventionOptions();
    ...
    public string FormatQueueName(Type consumerType)
    {
        ...
        string baseName = messageType != null
            ? FormatTypeName(messageType)
            : FormatTypeName(consumerType);

        string queueName = RemoveSuffix(baseName, "Consumer", "Handler", "Subscriber");
    ...
    public string FormatQueueNameFromMessage(Type messageType)
    {
        ...
        string baseName = FormatTypeName(messageType);
        string queueName = RemoveSuffix(baseName, "Message", "Event", "Command", "Query");
        queueName = $"{queueName}{Separator}queue";
    ...
    public string FormatExchangeName(Type messageType)
    {
        ...
        string baseName = FormatTypeName(messageType);
        string exchangeName = RemoveSuffix(baseName, "Message", "Event", "Command", "Query");
    ...
    public string FormatRoutingKey(Type messageType)
    {
        ...
        string[] namespaceParts = messageType.Namespace?.Split('.') ?? [];
        string typeName = FormatTypeName(messageType);
        ...
        if (_options.IncludeNamespaceInRoutingKey && namespaceParts.Length > 0)
        {
            ...
            return ApplyCasing(string.Join(Separator, relevantParts.Concat([typeName])));
        }
    ...
    public string FormatRetryQueueName(string originalQueueName, int retryLevel)
    {
        ...
        if (retryLevel < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(retryLevel), "Retry level must be >= 1");
        }
    ...
    public string SanitizeName(string name)
    {
        ...
        string sanitized = Regex.Replace(name, @"[^a-zA-Z0-9._\-]", string.Empty);
    ...
    private string FormatTypeName(Type type)
    {
        ...
        {
            string genericTypeDefinition = type.GetGenericTypeDefinition().Name;
            int index = genericTypeDefinition.IndexOf('`');
            if (index > 0)
            {
            ...
            string argNames = string.Join(Separator, genericArgs.Select(FormatTypeName));
