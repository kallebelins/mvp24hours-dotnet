...
{
    private readonly IEndpointNameFormatter _nameFormatter = nameFormatter ?? throw new ArgumentNullException(nameof(nameFormatter));
    private readonly IRoutingKeyConvention _routingKeyConvention = routingKeyConvention ?? throw new ArgumentNullException(nameof(routingKeyConvention));
    private readonly ITopologyBuilder _topologyBuilder = topologyBuilder ?? throw new ArgumentNullException(nameof(topologyBuilder));
    private readonly AutoBindingOptions _options = options ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<AutoBindingHelper>? _logger = logger;
    ...
    public ConsumerBindingInfo AutoBindConsumer(IModel channel, Type consumerType)
    {
        ...
        Type? messageType = GetMessageTypeFromConsumer(consumerType) ?? throw new InvalidOperationException(
        ...
        string queueName = GetQueueName(consumerType, messageType);
        string exchangeName = GetExchangeName(messageType);
        MvpRabbitMQExchangeType exchangeType = GetExchangeType(messageType);
        string routingKey = GetRoutingKey(consumerType, messageType, exchangeType);
    ...
    public MessageBindingInfo AutoBindMessage(IModel channel, Type messageType)
    {
        ...
        string exchangeName = GetExchangeName(messageType);
        MvpRabbitMQExchangeType exchangeType = GetExchangeType(messageType);
        string queueName = _nameFormatter.FormatQueueNameFromMessage(messageType);
        string routingKey = _routingKeyConvention.GetRoutingKey(messageType);
    ...
    private Dictionary<string, object> BuildQueueArguments(Type messageType, string exchangeName)
    {
        ...
        {
            string dlxName = _nameFormatter.FormatDeadLetterExchangeName(exchangeName);
