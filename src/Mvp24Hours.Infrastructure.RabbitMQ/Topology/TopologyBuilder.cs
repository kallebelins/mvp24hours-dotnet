...
public class TopologyBuilder(
    ...
    ILogger<TopologyBuilder>? logger = null) : ITopologyBuilder
{
    ...
    private readonly TopologyBuilderOptions _options = options ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<TopologyBuilder>? _logger = logger;
    ...
    public void ConfigureTopologyForMessage(IModel channel, Type messageType)
    {
        ...
        string exchangeName = topology?.ExchangeName ?? _nameFormatter.FormatExchangeName(messageType);
        MvpRabbitMQExchangeType exchangeType = topology?.ExchangeType ?? _options.DefaultExchangeType;
        bool durable = topology?.Durable ?? _options.DefaultDurable;
        bool autoDelete = topology?.AutoDelete ?? _options.DefaultAutoDelete;
        ...
        if (_options.AutoConfigureDeadLetter)
        {
            ...
            string dlqName = _nameFormatter.FormatDeadLetterQueueName(_nameFormatter.FormatQueueNameFromMessage(messageType));
    ...
    public void ConfigureTopologyForConsumer(IModel channel, Type consumerType)
    {
        ...
        IMessageTopology? topology = MessageTopologyRegistry.Instance.GetTopology(messageType);
        string exchangeName = topology?.ExchangeName ?? _nameFormatter.FormatExchangeName(messageType);
        string queueName = _nameFormatter.FormatQueueName(consumerType);
        string routingKey = _routingKeyConvention.GetSubscriptionPattern(consumerType, messageType);
        bool durable = topology?.Durable ?? _options.DefaultDurable;
