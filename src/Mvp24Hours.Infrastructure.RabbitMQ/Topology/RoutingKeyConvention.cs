...
public class RoutingKeyConvention(IEndpointNameFormatter nameFormatter, RoutingKeyConventionOptions options) : IRoutingKeyConvention
{
    ...
    private readonly RoutingKeyConventionOptions _options = options ?? throw new ArgumentNullException(nameof(options));
    ...
    public string GetRoutingKey(Type messageType)
    {
        ...
        // Add type name
        string typeName = GetTypeNameForRouting(messageType);
        parts.Add(typeName);

        // Add message type category if configured
        if (_options.IncludeMessageTypeCategory)
        {
            string category = DetermineMessageCategory(messageType, typeName);
        ...
        if (_options.UseWildcardsForBaseTypes)
        {
            ...
            while (baseType != null && baseType != typeof(object))
            {
                if (IsMessageBaseType(baseType))
                {
                    ...
                    string[] baseParts = GetRoutingKey(baseType).Split('.');
    ...
    private string GetTypeNameForRouting(Type type)
    {
        ...
        foreach (string? suffix in new[] { "Message", "Event", "Command", "Query", "Request", "Response" })
        {
