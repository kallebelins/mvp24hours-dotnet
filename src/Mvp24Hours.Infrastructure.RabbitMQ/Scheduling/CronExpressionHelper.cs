...
public static class CronExpressionHelper
{
    ...
    public static string GetDescription(string cronExpression)
    {
        ...
        return commonExpressions.TryGetValue(cronExpression, out string? description)
            ? description
    ...
    private static HashSet<int> ParseField(string field, int min, int max)
    {
        ...
        foreach (string part in field.Split(','))
        {
            ...
            else if (part.Contains('/'))
            {
                ...
                int step = int.Parse(stepParts[1], CultureInfo.InvariantCulture);
            ...
            {
                // Range: 1-5
                foreach (int value in ParseRange(part, min, max))
                {
                    result.Add(value);
                }
            }
            ...
            {
                // Single value
                int value = int.Parse(part, CultureInfo.InvariantCulture);
    ...
    private static IEnumerable<int> ParseRange(string part, int min, int max)
    {
        ...
        int start = int.Parse(rangeParts[0], CultureInfo.InvariantCulture);
        int end = int.Parse(rangeParts[1], CultureInfo.InvariantCulture);
