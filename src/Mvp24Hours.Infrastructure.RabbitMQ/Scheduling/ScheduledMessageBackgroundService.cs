...
public class ScheduledMessageBackgroundService(
    ...
    ILogger<ScheduledMessageBackgroundService>? logger = null) : BackgroundService
{
    private readonly MessageScheduler _scheduler = scheduler ?? throw new ArgumentNullException(nameof(scheduler));
    private readonly IScheduledMessageStore _store = store ?? throw new ArgumentNullException(nameof(store));
    private readonly MessageSchedulerOptions _options = options?.Value ?? new MessageSchedulerOptions();
    private readonly ILogger<ScheduledMessageBackgroundService>? _logger = logger;
    ...
    {
        int processed = await _scheduler.ProcessDueMessagesAsync(stoppingToken);
    ...
    private async Task CleanupOldMessagesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            int removed = await _store.CleanupOldMessagesAsync(olderThan, cancellationToken);
