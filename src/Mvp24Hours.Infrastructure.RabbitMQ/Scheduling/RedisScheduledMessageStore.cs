...
public class RedisScheduledMessageStore : IScheduledMessageStore
{
    ...
    public async Task UpdateAsync(ScheduledMessage message, CancellationToken cancellationToken = default)
    {
        ...
        await _cache.SetStringAsync(key, json, new DistributedCacheEntryOptions
        {
            AbsoluteExpirationRelativeToNow = message.Status is ScheduledMessageStatus.Completed or
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (Guid id in _messageIndex.Keys.Take(batchSize * 10)) // Check more than batch size
        {
            ...
            bool isDue = (message.Status == ScheduledMessageStatus.Pending || message.Status == ScheduledMessageStatus.Active)
    ...
    {
        // Get current index
        string? indexJson = await _cache.GetStringAsync(INDEX_KEY, cancellationToken);
