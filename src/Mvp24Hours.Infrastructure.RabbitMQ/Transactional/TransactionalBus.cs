...
public class TransactionalBus : ITransactionalBus
{
    ...
    {
        int count = _pendingMessages.Value?.Count ?? 0;
    ...
    {
        if (headers.TryGetValue("x-correlation-id", out object? value) ||
    ...
    {
        if (headers.TryGetValue("x-causation-id", out object? value) ||
    ...
    {
        if (headers.TryGetValue("x-priority", out object? value) ||
            ...
            headers.TryGetValue("Priority", out value))
        {
            ...
            if (byte.TryParse(value?.ToString(), out byte parsed))
            {
                return parsed;
            }
        }
    ...
    {
        if (headers.TryGetValue("x-tenant-id", out object? value) ||
