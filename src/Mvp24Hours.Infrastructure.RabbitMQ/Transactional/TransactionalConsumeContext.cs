...
public class TransactionalConsumeContext<TMessage>(
    IConsumeContext<TMessage> innerContext,
    ITransactionalBus transactionalBus) : ITransactionalConsumeContext<TMessage>
    where TMessage : class
{
    private readonly IConsumeContext<TMessage> _innerContext = innerContext ?? throw new ArgumentNullException(nameof(innerContext));

    /// <inheritdoc />
    public ITransactionalBus TransactionalBus { get; } = transactionalBus ?? throw new ArgumentNullException(nameof(transactionalBus));
    ...
    {
        return _innerContext.GetHeader<T>(key);
    }
        ...
        where T : class
    {
        ...
        return await TransactionalBus.PublishAsync(message, headers, routingKey, cancellationToken);
    }
        ...
        where T : class
    {
        ...
        return await TransactionalBus.PublishAsync(message, mergedHeaders, routingKey, cancellationToken);
    }
...
public class TransactionalConsumeContextFactory(ITransactionalBus transactionalBus) : ITransactionalConsumeContextFactory
{
    private readonly ITransactionalBus _transactionalBus = transactionalBus ?? throw new ArgumentNullException(nameof(transactionalBus));
