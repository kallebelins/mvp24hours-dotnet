...
public sealed class InMemoryTransactionalOutbox(
    ILogger<InMemoryTransactionalOutbox>? logger = null,
    InMemoryTransactionalOutboxOptions? options = null) : ITransactionalOutbox
{
    private readonly ConcurrentDictionary<Guid, TransactionalOutboxMessage> _messages = new();
    private readonly ILogger<InMemoryTransactionalOutbox>? _logger = logger;
    private readonly InMemoryTransactionalOutboxOptions _options = options ?? new InMemoryTransactionalOutboxOptions();
    ...
    {
        int count = _messages.Values.Count(m =>
            m.Status is TransactionalOutboxStatus.Pending or
            TransactionalOutboxStatus.Failed);
