...
public class TransactionalUnitOfWork(
    ...
    TransactionalBus transactionalBus) : IUnitOfWork
{
    private readonly IUnitOfWork _innerUnitOfWork = innerUnitOfWork ?? throw new ArgumentNullException(nameof(innerUnitOfWork));
    private readonly TransactionalBus _transactionalBus = transactionalBus ?? throw new ArgumentNullException(nameof(transactionalBus));
...
/// <param name="transactionalBus">The transactional bus for message staging.</param>
public class TransactionalUnitOfWorkAsync(
    IUnitOfWorkAsync innerUnitOfWork,
    TransactionalBus transactionalBus) : IUnitOfWorkAsync
{
    private readonly IUnitOfWorkAsync _innerUnitOfWork = innerUnitOfWork ?? throw new ArgumentNullException(nameof(innerUnitOfWork));
    private readonly TransactionalBus _transactionalBus = transactionalBus ?? throw new ArgumentNullException(nameof(transactionalBus));
...
public class TransactionalUnitOfWorkFactory(TransactionalBus transactionalBus) : ITransactionalUnitOfWorkFactory
{
    private readonly TransactionalBus _transactionalBus = transactionalBus ?? throw new ArgumentNullException(nameof(transactionalBus));
