...
public class TransactionalEnlistment(
    ITransactionalOutbox outbox,
    ILogger<TransactionalEnlistment> logger) : ITransactionalEnlistment
{
    private readonly ITransactionalOutbox _outbox = outbox ?? throw new ArgumentNullException(nameof(outbox));
    private readonly ILogger<TransactionalEnlistment> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
...
/// </remarks>
internal sealed class TransactionalBusEnlistmentNotification(
    TransactionalBus bus,
    ITransactionalOutbox outbox,
    ILogger logger) : IEnlistmentNotification
{
    private readonly TransactionalBus _bus = bus;
    private readonly ITransactionalOutbox _outbox = outbox;
    ...
    public void Prepare(PreparingEnlistment preparingEnlistment)
    {
        try
        {
            int pendingCount = _bus.GetPendingCount();
            _logger.LogDebug(
