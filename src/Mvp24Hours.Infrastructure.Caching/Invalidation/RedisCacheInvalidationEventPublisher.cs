...
public class RedisCacheInvalidationEventPublisher : ICacheInvalidationEventPublisher
{
    ...
    public async Task PublishKeyInvalidationAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            string message = JsonSerializer.Serialize(new { Key = key, Timestamp = DateTimeOffset.UtcNow });
    ...
    public async Task PublishTagInvalidationAsync(string tag, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            string message = JsonSerializer.Serialize(new { Tag = tag, Timestamp = DateTimeOffset.UtcNow });
    ...
    public async Task PublishTagsInvalidationAsync(string[] tags, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            string message = JsonSerializer.Serialize(new { Tags = tags, Timestamp = DateTimeOffset.UtcNow });
            await PublishToRedisAsync(TagInvalidationChannel, message, cancellationToken);
    ...
    private async Task PublishToRedisAsync(string channel, string message, CancellationToken cancellationToken)
    {
        try
        {
            ...
            MethodInfo? getDatabaseMethod = connectionType.GetMethod("GetDatabase", [typeof(int)]) ?? connectionType.GetMethod("GetDatabase");
            ...
            {
                database = getDatabaseMethod.Invoke(_redisConnection, [-1]);
            }
            else
            {
            ...
            MethodInfo? publishMethod = database.GetType().GetMethod("PublishAsync", [typeof(string), typeof(string)]);
            if (publishMethod != null)
            {
                if (publishMethod.Invoke(database, [channel, message]) is Task<long> task)
                {
                    await task;
                    return;
                }
            ...
            if (publishMethod != null)
            {
                ...
                {
                    string channelParam = channel; // Use string directly
                    if (publishMethod.Invoke(database, [channelParam, message]) is Task task)
                    {
                        await task;
                        return;
                    }
    ...
    private bool CheckRedisAvailability()
    {
        try
        {
            ...
            {
                object? isConnected = isConnectedProperty.GetValue(_redisConnection);
