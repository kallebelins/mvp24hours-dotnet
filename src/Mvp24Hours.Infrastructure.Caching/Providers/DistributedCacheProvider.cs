...
public class DistributedCacheProvider(
    ...
    ILogger<DistributedCacheProvider>? logger = null) : ICacheProvider
{
    private readonly IDistributedCache _cache = cache ?? throw new ArgumentNullException(nameof(cache));
    private readonly ICacheSerializer _serializer = serializer ?? new JsonCacheSerializer();
    private readonly ILogger<DistributedCacheProvider>? _logger = logger;
    ...
    public async Task SetAsync<T>(string key, T value, CacheEntryOptions? options = null, CancellationToken cancellationToken = default) where T : class
    {
        ...
        try
        {
            byte[] bytes = await _serializer.SerializeAsync(value, cancellationToken);
