...
public class MultiLevelCache : IMultiLevelCache
{
    ...
    public async Task<bool> DemoteFromL1Async(string key, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            bool exists = await _l1Cache.ExistsAsync(key, cancellationToken);
            if (!exists)
            {
                return false;
            }
    ...
    public async Task<string?> GetStringAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Try L1 first
            string? l1Value = await _l1Cache.GetStringAsync(key, cancellationToken);
            if (l1Value != null)
            {
                _statistics.L1.Hits++;
                return l1Value;
            ...
            // Try L2
            string? l2Value = await _l2Cache.GetStringAsync(key, cancellationToken);
            if (l2Value != null)
            {
                _statistics.L2.Hits++;
                // Promote to L1
    ...
    public async Task<bool> ExistsAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        bool existsInL1 = await _l1Cache.ExistsAsync(key, cancellationToken);
        if (existsInL1)
        {
            return true;
        }
    ...
    public async Task<Dictionary<string, T>> GetManyAsync<T>(string[] keys, CancellationToken cancellationToken = default) where T : class
    {
        ...
        string[] missingKeys = [.. keys.Where(k => !result.ContainsKey(k))];
