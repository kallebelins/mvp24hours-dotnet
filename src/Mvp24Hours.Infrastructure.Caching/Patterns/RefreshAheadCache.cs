...
internal class CachedItemWithExpiration<T>(T value, TimeSpan expiration) where T : class
{
    ...
    {
        return DateTime.UtcNow >= ExpiresAt.Subtract(refreshThreshold);
    }
}
...
public class RefreshAheadCache<T>(
    ...
    ILogger<RefreshAheadCache<T>>? logger = null) : IRefreshAheadCache<T> where T : class
{
    private readonly ICacheProvider _cache = cache ?? throw new ArgumentNullException(nameof(cache));
    private readonly Func<string, CancellationToken, Task<T?>> _loadFromSource = loadFromSource ?? throw new ArgumentNullException(nameof(loadFromSource));
    private readonly TimeSpan _expiration = expiration;
    private readonly TimeSpan _refreshThreshold = refreshThreshold;
    private readonly Func<string, CacheEntryOptions>? _getCacheOptions = getCacheOptions;
    private readonly ILogger<RefreshAheadCache<T>>? _logger = logger;
