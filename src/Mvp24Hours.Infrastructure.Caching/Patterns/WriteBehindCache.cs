...
internal class PendingWrite<T>(string key, T value) where T : class
{
...
public class WriteBehindCache<T>(
    ...
    ILogger<WriteBehindCache<T>>? logger = null) : IWriteBehindCache<T> where T : class
{
    private readonly ICacheProvider _cache = cache ?? throw new ArgumentNullException(nameof(cache));
    private readonly Func<string, T, CancellationToken, Task> _saveToSource = saveToSource ?? throw new ArgumentNullException(nameof(saveToSource));
    private readonly Func<string, CacheEntryOptions>? _getCacheOptions = getCacheOptions;
    private readonly ILogger<WriteBehindCache<T>>? _logger = logger;
    private readonly ConcurrentQueue<PendingWrite<T>> _writeQueue = new();
