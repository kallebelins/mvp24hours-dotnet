    ...
    where TEntity : class, IEntityBase
{
    ...
    private readonly ICacheKeyGenerator _keyGenerator = keyGenerator ?? new DefaultCacheKeyGenerator(typeof(TEntity).Name);
    private readonly ILogger<CacheableRepository<TEntity>>? _logger = logger;
    private readonly CacheableRepositoryOptions _options = options ?? new CacheableRepositoryOptions();
        ...
        object? parameters) where TResult : class
    {
        ...
        string cacheKey = GenerateCacheKey(methodName, parameters, cacheableAttr);
        int duration = cacheableAttr?.DurationSeconds ?? _options.DefaultCacheDurationSeconds;
    ...
    private string GenerateCacheKey(string methodName, object? parameters, CacheableAttribute? attribute)
    {
        ...
        {
            // Use custom template
            string key = attribute.KeyTemplate;
        ...
        return _keyGenerator.Generate([.. keyParts]);
    }
    ...
    private void InvalidateCacheForEntityId(object id)
    {
        ...
        try
        {
            ...
            string pattern = $"{typeof(TEntity).Name}:*:{id}";
