...
public class HybridCacheProvider(
    ...
    IHybridCacheTagManager? tagManager = null) : ICacheProvider
{
    private readonly Microsoft.Extensions.Caching.Hybrid.HybridCache _hybridCache = hybridCache ?? throw new ArgumentNullException(nameof(hybridCache));
    private readonly MvpHybridCacheOptions _options = options?.Value ?? new MvpHybridCacheOptions();
    private readonly ILogger<HybridCacheProvider>? _logger = logger;
    private readonly IHybridCacheTagManager? _tagManager = tagManager;
    ...
    public async Task<bool> ExistsAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        object? result = await GetAsync<object>(key, cancellationToken);
        return result != null;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string fullKey = GetFullKey(key);
        HybridCacheEntryOptions? entryOptions = ConvertToHybridCacheEntryOptions(options);
        string[]? allTags = GetTags(options, tags);
    ...
    private string[]? GetTags(CacheEntryOptions? options, string[]? additionalTags = null)
    {
        ...
        return tags.Count > 0 ? [.. tags.Distinct()] : null;
    }
