...
public class RedisHybridCacheTagManager : IHybridCacheTagManager
{
    ...
    public async Task TrackKeyWithTagsAsync(string key, string[] tags, CancellationToken cancellationToken = default)
    {
        ...
        string keyTagsKey = GetKeyTagsKey(fullKey);

        try
        {
            ITransaction transaction = _database.CreateTransaction();

            foreach (string? tag in tags.Where(t => !string.IsNullOrWhiteSpace(t)))
            {
    ...
    public async Task RemoveKeyFromTagsAsync(string key, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (tags.Length > 0)
            {
                ...
                {
                    string tagKey = GetTagKey(tag.ToString());
        ...
        try
        {
            ...
            if (keys.Length > 0)
            {
                ...
                foreach (RedisValue keyValue in keys)
                {
                    ...
                    string keyTagsKey = GetKeyTagsKey(keyValue.ToString());
    ...
    public HybridCacheTagStatistics GetStatistics()
    {
        try
        {
            ...
            if (invalidations.HasValue && long.TryParse(invalidations.ToString(), out long count))
            {
            ...
            {
                int keyCount = (int)_database.SetLength(tagKey);
                string tagName = tagKey.ToString().Replace($"{_options.KeyPrefix}{TagPrefix}", "");
    ...
    public async Task ClearAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            RedisKey[] tagKeys = [.. server.Keys(pattern: $"{_options.KeyPrefix}{TagPrefix}*", pageSize: 1000)];
            RedisKey[] keyTagsKeys = [.. server.Keys(pattern: $"{_options.KeyPrefix}{KeyTagsPrefix}*", pageSize: 1000)];
    ...
    private string RemoveKeyPrefix(string fullKey)
    {
        ...
        return fullKey.StartsWith(_options.KeyPrefix)
            ? fullKey[_options.KeyPrefix.Length..]
            : fullKey;
    }
