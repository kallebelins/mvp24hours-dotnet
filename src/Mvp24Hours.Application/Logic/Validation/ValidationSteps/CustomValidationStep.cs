...
public abstract class CustomValidationStep<T> : IValidationStep<T> where T : class
{
    ...
    {
        string fullPath = context.Options.IncludePropertyPath && !string.IsNullOrEmpty(context.PropertyPath)
...
public class PredicateValidationStep<T>(
    string name,
    Func<T, ValidationStepContext, ValidationServiceResult> validateFunc,
    int order = 500,
    Func<T, ValidationStepContext, bool>? shouldExecuteFunc = null) : IValidationStep<T> where T : class
{
    private readonly Func<T, ValidationStepContext, ValidationServiceResult> _validateFunc = validateFunc ?? throw new ArgumentNullException(nameof(validateFunc));
    private readonly Func<T, ValidationStepContext, Task<ValidationServiceResult>>? _validateFuncAsync;
    private readonly Func<T, ValidationStepContext, bool>? _shouldExecuteFunc = shouldExecuteFunc;
...
/// <param name="name">The name of this validation step.</param>
/// <param name="order">The order of execution (default: 500).</param>
public class RuleBasedValidationStep<T>(string name, int order = 500) : IValidationStep<T> where T : class
{
