...
public class CursorPagedResult<T> : ICursorPagedResult<T>
{
    ...
    {
        return new CursorPagedResult<T>([], pageSize);
    }
...
public class CursorPagedResult<T, TCursor>(
    ...
    bool hasNextPage = false,
    bool hasPreviousPage = false) : CursorPagedResult<T>(
        items,
        pageSize,
        SerializeCursor(nextCursorValue),
        SerializeCursor(previousCursorValue),
        hasNextPage,
        hasPreviousPage), ICursorPagedResult<T, TCursor>
    where TCursor : struct
{

    /// <inheritdoc/>
    [DataMember]
    public TCursor? NextCursorValue { get; init; } = nextCursorValue;

    /// <inheritdoc/>
    [DataMember]
    public TCursor? PreviousCursorValue { get; init; } = previousCursorValue;
    ...
    private static string? SerializeCursor(TCursor? cursorValue)
    {
        ...
        string json = JsonSerializer.Serialize(cursorValue.Value);
    ...
    public static TCursor? DeserializeCursor(string? cursorString)
    {
        ...
        try
        {
            string json = Encoding.UTF8.GetString(Convert.FromBase64String(cursorString));
...
public class CompositeCursor : ICompositeCursor
{
    ...
    {
        if (_fields.TryGetValue(name, out object? value) && value is TValue typedValue)
        {
            return typedValue;
        }
        return default;
    ...
    {
        string json = JsonSerializer.Serialize(_fields);
        return Convert.ToBase64String(Encoding.UTF8.GetBytes(json));
    }
    ...
    /// </summary>
    public static CompositeCursor Create()
    {
        return new();
    }
