...
public class BusinessResultWithStatus<T>(
    T? data = default,
    ResultStatusCode statusCode = ResultStatusCode.Success,
    IEnumerable<IResultMessage>? messages = null,
    string? token = null) : BaseVO, IBusinessResultWithStatus<T>
{
    #region [ Fields ]

    private readonly List<IResultMessage> _extendedMessages = messages?.ToList() ?? [];
    ...
    public ResultStatusCode StatusCode { get; } = statusCode;
    ...
    public IReadOnlyCollection<IMessageResult>? Messages =>
        _extendedMessages.Count > 0
            ? new ReadOnlyCollection<IMessageResult>([.. _extendedMessages.Cast<IMessageResult>()])
            : null;
