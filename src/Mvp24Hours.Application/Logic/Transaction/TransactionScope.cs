...
public sealed class TransactionScope(IUnitOfWorkAsync unitOfWork, ILogger<TransactionScope>? logger = null) : ITransactionScope
{
    #region [ Fields ]

    private readonly IUnitOfWorkAsync _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
    private readonly ILogger<TransactionScope>? _logger = logger;
    ...
    public async Task<int> CommitAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                string savepoint = _savepoints.Pop();
    ...
    public async Task RollbackToSavepointAsync(string savepointName, CancellationToken cancellationToken = default)
    {
        ...
        while (_savepoints.Count > 0)
        {
            string current = _savepoints.Pop();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            int affectedRows = await CommitAsync(cancellationToken);
...
/// <param name="unitOfWork">The unit of work to manage transactions for.</param>
/// <param name="logger">Optional logger for recording transaction operations.</param>
public sealed class TransactionScopeSync(IUnitOfWork unitOfWork, ILogger<TransactionScopeSync>? logger = null) : ITransactionScopeSync
{
    #region [ Fields ]

    private readonly IUnitOfWork _unitOfWork = unitOfWork ?? throw new ArgumentNullException(nameof(unitOfWork));
    private readonly ILogger<TransactionScopeSync>? _logger = logger;
    private bool _disposed;
