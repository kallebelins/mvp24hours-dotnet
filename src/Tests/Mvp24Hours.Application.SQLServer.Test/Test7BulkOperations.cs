...
public class Test7BulkOperations
{
    ...
    public Test7BulkOperations()
    {
        ...
        services.AddDbContext<DataContext>(options => options.UseInMemoryDatabase($"BulkOperationsTest_{Guid.NewGuid()}"));
    ...
    public async Task BulkUpdate_ShouldUpdateAllEntities()
    {
        ...
        // Act
        BulkOperationResult result = await repository.BulkUpdateAsync([.. insertedCustomers]);
        await unitOfWork.SaveChangesAsync();
    ...
    public async Task BulkDelete_ShouldDeleteAllEntities()
    {
        ...
        // Act
        BulkOperationResult result = await repository.BulkDeleteAsync([.. insertedCustomers]);
        await unitOfWork.SaveChangesAsync();
    ...
    public async Task ExecuteDelete_ShouldDeleteMatchingEntities()
    {
        ...
        int rowsAffected = await repository.ExecuteDeleteAsync(c => c.Active == false);
    ...
    public async Task ExecuteDelete_WithNoMatches_ShouldReturnZero()
    {
        ...
        int rowsAffected = await repository.ExecuteDeleteAsync(c => c.Name == "NonExistentName12345");
    ...
    public async Task DbContextExtension_ExecuteDelete_ShouldWork()
    {
        ...
        // Act
        int rowsAffected = await dbContext.ExecuteDeleteAsync<Customer>(c => c.Name.StartsWith("Customer_"));
