...
public class HybridCacheTest
{
    ...
    public async Task TagManager_TrackKeyWithTags_ShouldTrackCorrectly()
    {
        ...
        string[] tags = ["products", "category:electronics"];
    ...
    public async Task TagManager_InvalidateTag_ShouldRemoveAllAssociations()
    {
        // Arrange
        var tagManager = new InMemoryHybridCacheTagManager();
        await tagManager.TrackKeyWithTagsAsync("product:1", ["products"]);
        await tagManager.TrackKeyWithTagsAsync("product:2", ["products"]);
        await tagManager.TrackKeyWithTagsAsync("product:3", ["products", "featured"]);
    ...
    public async Task TagManager_GetStatistics_ShouldReturnCorrectStats()
    {
        // Arrange
        var tagManager = new InMemoryHybridCacheTagManager();
        await tagManager.TrackKeyWithTagsAsync("product:1", ["products", "featured"]);
        await tagManager.TrackKeyWithTagsAsync("product:2", ["products"]);
        await tagManager.TrackKeyWithTagsAsync("category:1", ["categories"]);
    ...
    public async Task TagManager_TrackKeyWithTags_NullKey_ShouldThrow()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            tagManager.TrackKeyWithTagsAsync(null!, ["tag"]));
    }
    ...
    public async Task TagManager_TrackKeyWithTags_EmptyTags_ShouldNotThrow()
    {
        ...
        // Act
        await tagManager.TrackKeyWithTagsAsync("key", []);
    ...
    public async Task TagManager_MultipleKeysWithSameTag_ShouldTrackAll()
    {
        ...
        for (int i = 1; i <= 100; i++)
        {
            await tagManager.TrackKeyWithTagsAsync($"product:{i}", [tag]);
        }
...
#pragma warning restore EXTEXP0018
    ...
    public async Task TagManager_ConcurrentAccess_ShouldBeThreadSafe()
    {
        ...
        {
            int index = i;
            tasks.Add(Task.Run(async () => await tagManager.TrackKeyWithTagsAsync($"key:{index}", ["tag1", "tag2"])));
        }
    ...
    public async Task TagManager_InvalidateTagDuringWrite_ShouldNotThrow()
    {
        // Arrange
        var tagManager = new InMemoryHybridCacheTagManager();
        await tagManager.TrackKeyWithTagsAsync("key:1", ["tag1"]);

        // Act - Concurrent invalidation and write
        Task[] tasks =
        [
            tagManager.InvalidateTagAsync("tag1"),
            tagManager.TrackKeyWithTagsAsync("key:2", ["tag1"])
        ];
    ...
    public async Task ContainsKeyAsync_ShouldCallExistsAsync()
    {
        ...
        // Act
        bool result = await mockProvider.Object.ContainsKeyAsync("key");
    ...
    public async Task RemoveByPrefixAsync_ShouldFilterAndRemoveMatchingKeys()
    {
        // Arrange
        var mockProvider = new Mock<ICacheProvider>();
        string[] keys = ["product:1", "product:2", "category:1", "product:3"];
    ...
    public async Task InvalidateByTagsAsync_WithNonHybridCacheProvider_ShouldThrow()
    {
        ...
        // Act & Assert
        InvalidOperationException exception = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            mockProvider.Object.InvalidateByTagsAsync(["tag1", "tag2"]));
        exception.Message.Should().Contain("HybridCacheProvider");
    }
    ...
    public async Task FullTaggingWorkflow_ShouldWorkCorrectly()
    {
        ...
        await tagManager.TrackKeyWithTagsAsync("product:1", ["products", "category:electronics", "featured"]);
        await tagManager.TrackKeyWithTagsAsync("product:2", ["products", "category:electronics"]);
        await tagManager.TrackKeyWithTagsAsync("product:3", ["products", "category:clothing"]);
        await tagManager.TrackKeyWithTagsAsync("product:4", ["products", "category:clothing", "featured"]);
    ...
    public async Task RemoveKey_ShouldCleanUpAllTagAssociations()
    {
        // Arrange
        var tagManager = new InMemoryHybridCacheTagManager();
        await tagManager.TrackKeyWithTagsAsync("product:1", ["products", "featured", "new-arrivals"]);
