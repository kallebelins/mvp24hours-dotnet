...
/// <summary>
/// Hangfire provider is currently a stub (Hangfire packages not referenced).
/// Tests cover constructor guards and NotSupportedException on all operations.
/// </summary>
[Trait("Category", "Unit")]
public class HangfireJobProviderTest
{
    [Fact]
    public void Constructor_WithNullServiceProvider_ShouldThrowArgumentNullException()
    {
        Action act = () => _ = new HangfireJobProvider(
            null!,
            BackgroundJobsTestHelpers.CreateHangfireOptions());
    ...
    {
        HangfireJobProvider provider = CreateProvider();
    ...
    {
        var logger = new Mock<ILogger<HangfireJobProvider>>();
    ...
    public async Task EnqueueAsync_WithArgs_ShouldThrowNotSupportedException()
    {
        ...
        await act.Should().ThrowAsync<NotSupportedException>().WithMessage("*Hangfire*");
    }
    ...
    private static HangfireJobProvider CreateProvider(IOptions<HangfireJobOptions>? options = null)
    {
        return new HangfireJobProvider(
            new ServiceCollection().BuildServiceProvider(),
