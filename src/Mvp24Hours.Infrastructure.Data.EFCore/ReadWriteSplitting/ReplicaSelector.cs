...
public class ReplicaSelector : IReplicaSelector
{
    ...
    {
        foreach (string connectionString in _options.ReplicaConnectionStrings)
        {
    ...
    public Task<string?> SelectReplicaAsync(CancellationToken cancellationToken = default)
    {
        ...
        string selected = _options.LoadBalancing switch
        {
    ...
    {
        int index = Interlocked.Increment(ref _roundRobinIndex) % replicas.Count;
        return replicas[index];
    }

    private string SelectRandom(List<string> replicas)
    {
        int index = _random.Next(replicas.Count);
        return replicas[index];
    }
    ...
    private string SelectWeighted(List<string> replicas)
    {
        ...
        {
            int originalIndex = _options.ReplicaConnectionStrings.IndexOf(replicas[i]);
            int weight = originalIndex >= 0 && originalIndex < _options.ReplicaWeights.Count
