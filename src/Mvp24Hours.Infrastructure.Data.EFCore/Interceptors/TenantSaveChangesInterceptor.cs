...
public class TenantSaveChangesInterceptor(
    ...
    TenantInterceptorOptions? options = null) : SaveChangesInterceptor
{
    private readonly ITenantProvider _tenantProvider = tenantProvider ?? throw new ArgumentNullException(nameof(tenantProvider));
    private readonly TenantInterceptorOptions _options = options ?? new TenantInterceptorOptions();
    ...
    private void ProcessTenantEntities(DbContext? context)
    {
        ...
        {
            bool hasTenantEntities = context.ChangeTracker
                .Entries()
    ...
    private void TryProcessGenericTenantEntity(EntityEntry entry, string currentTenantId)
    {
        ...
        foreach (Type iface in interfaces)
        {
            if (iface.IsGenericType && iface.GetGenericTypeDefinition() == typeof(ITenantEntity<>))
            {
                ...
                {
                    case EntityState.Added:
                        object? currentValue = tenantIdProperty.CurrentValue;
                        if (currentValue == null || IsDefaultValue(currentValue))
                        {
                                     ...
                                     Guid.TryParse(currentTenantId, out Guid guidTenantId))
                            {
                                tenantIdProperty.CurrentValue = guidTenantId;
                                     ...
                                     int.TryParse(currentTenantId, out int intTenantId))
                            {
