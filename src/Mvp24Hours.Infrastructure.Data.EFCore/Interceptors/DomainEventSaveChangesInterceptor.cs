...
public class DomainEventSaveChangesInterceptor(
    IDomainEventDispatcherEFCore? eventDispatcher = null,
    ILogger<DomainEventSaveChangesInterceptor>? logger = null) : SaveChangesInterceptor
{
    private readonly IDomainEventDispatcherEFCore? _eventDispatcher = eventDispatcher;
    private readonly ILogger<DomainEventSaveChangesInterceptor>? _logger = logger;
    ...
    private void CollectDomainEvents(DbContext? context)
    {
        ...
        _pendingEvents = [.. entitiesWithEvents.Select(e => (Entity: e, Events: e.DomainEvents.ToList()))];

        int totalEvents = _pendingEvents.Sum(p => p.Events.Count);
