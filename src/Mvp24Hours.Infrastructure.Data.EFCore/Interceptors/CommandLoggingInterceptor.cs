...
public class CommandLoggingInterceptor : DbCommandInterceptor
{
...
#endif
        _sensitiveParameters = sensitiveParameters ?? ["password", "pwd", "token", "secret", "key", "credential"];
    }
    ...
    {
        bool isSlowQuery = duration >= _slowQueryThreshold;
        ...
        if (_logParameters && command.Parameters.Count > 0)
        {
            ...
            {
                string value = IsSensitiveParameter(param.ParameterName) ? "***MASKED***" : param.Value?.ToString() ?? "NULL";
    ...
    private bool IsSensitiveParameter(string parameterName)
    {
        ...
        string normalizedName = parameterName.ToLowerInvariant();
