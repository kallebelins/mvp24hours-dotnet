...
public static class ResilienceDbContextExtensions
{
    ...
    /// <item>EF Core built-in <c>SqlServerRetryingExecutionStrategy</c> (via <c>EnableRetryOnFailure</c>)</item>
    /// <item>Optional DbContext pooling</item>
    /// <item>Command timeout settings</item>
    /// </list>
    /// </para>
    /// <para>
    /// For Polly-based resilience (circuit breaker, custom pipelines), prefer
    /// <see cref="NativeDbResilienceExtensions"/>.
    /// </para>
    /// </remarks>
    /// <example>
    /// <code>
        ...
        where TDbContext : DbContext
    {
        ...
        return services.AddMvp24HoursDbContextWithResilience<TDbContext>(
            ...
            options =>
            {
                ...
                foreach (int errorNumber in azureOptions.AdditionalTransientErrorNumbers)
                {
    ...
    {
        // serviceProvider retained for public API compatibility (UseSqlServerWithResilience);
        // unused after migrating off obsolete MvpExecutionStrategy (§1.B / task 7.1).
        _ = serviceProvider;
        ...
        options.UseSqlServer(connectionString, sqlOptions =>
        {
            ...
            // Prefer EF Core's built-in SqlServerRetryingExecutionStrategy over the
            // obsolete MvpExecutionStrategy. For Polly pipelines, use AddNativeDbResilience.
