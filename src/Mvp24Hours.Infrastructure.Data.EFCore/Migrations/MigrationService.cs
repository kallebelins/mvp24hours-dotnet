...
public class MigrationService<TContext>(
    TContext dbContext,
    IOptions<MigrationOptions> options,
    ILogger<MigrationService<TContext>> logger) : IMigrationService where TContext : DbContext
{
    ...
    private readonly ILogger<MigrationService<TContext>> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task<MigrationResult> RollbackLastAsync(CancellationToken cancellationToken = default)
    {
        ...
        string targetMigration = appliedMigrations[^2]; // Second to last
    ...
    public async Task<bool> DeleteDatabaseAsync(CancellationToken cancellationToken = default)
    {
        ...
        bool deleted = await _dbContext.Database.EnsureDeletedAsync(cancellationToken);
