...
public class MigrationHostedService<TContext>(
    IServiceProvider serviceProvider,
    IOptions<MigrationOptions> options,
    ILogger<MigrationHostedService<TContext>> logger) : IHostedService
    where TContext : Microsoft.EntityFrameworkCore.DbContext
{
    private readonly IServiceProvider _serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
    private readonly MigrationOptions _options = options?.Value ?? new MigrationOptions();
    private readonly ILogger<MigrationHostedService<TContext>> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            bool hasPending = await migrationService.HasPendingMigrationsAsync(cancellationToken);
...
/// <param name="logger">Logger for diagnostic output.</param>
public abstract class DataSeederBase<TContext>(TContext dbContext, ILogger logger) : IDataSeeder
    where TContext : Microsoft.EntityFrameworkCore.DbContext
{
    /// <summary>
    /// Gets the DbContext for seeding operations.
    /// </summary>
    protected TContext DbContext { get; } = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
