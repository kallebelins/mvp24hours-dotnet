...
public class DbContextHealthCheck<TContext>(
    ...
    ILogger<DbContextHealthCheck<TContext>> logger) : IHealthCheck where TContext : DbContext
{
    private readonly TContext _dbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
    private readonly DbContextHealthCheckOptions _options = options?.Value ?? new DbContextHealthCheckOptions();
    private readonly ILogger<DbContextHealthCheck<TContext>> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
