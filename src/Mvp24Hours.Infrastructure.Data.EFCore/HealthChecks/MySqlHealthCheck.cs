...
public class MySqlHealthCheck(
    ...
    Func<string, DbConnection> connectionFactory) : IHealthCheck
{
    private readonly string _connectionString = connectionString ?? throw new ArgumentNullException(nameof(connectionString));
    private readonly MySqlHealthCheckOptions _options = options ?? new MySqlHealthCheckOptions();
    private readonly ILogger<MySqlHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                int current = Convert.ToInt32(status["Threads_connected"]);
                int max = Convert.ToInt32(status["max_connections"]);
                ...
                double usageRatio = max > 0 ? (double)current / max : 0;
            ...
            {
                long slowQueries = Convert.ToInt64(status["Slow_queries"]);
            ...
            {
                long lockWaits = Convert.ToInt64(status["Table_locks_waited"]);
            ...
            {
                long questions = Convert.ToInt64(status["Questions"]);
                long uptime = Convert.ToInt64(status["Uptime"]);
                if (uptime > 0)
                {
            ...
            if (_options.CheckReplication)
            {
                ...
                if (replicationStatus != null)
                {
                    ...
                    if (replicationStatus.TryGetValue("Seconds_Behind_Master", out object? lag) &&
                        lag != null &&
    ...
    private static async Task<Dictionary<string, object>> GetBufferPoolStatsAsync(DbConnection connection, CancellationToken cancellationToken)
    {
        ...
        if (stats.TryGetValue("Innodb_buffer_pool_read_requests", out object? requests) &&
            stats.TryGetValue("Innodb_buffer_pool_reads", out object? reads))
        {
            long requestsLong = Convert.ToInt64(requests);
            long readsLong = Convert.ToInt64(reads);
            if (requestsLong > 0)
            {
                double hitRatio = Math.Round((1.0 - (double)readsLong / requestsLong) * 100, 2);
...
public sealed class MySqlHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["db", "database", "mysql", "ready"];
}
