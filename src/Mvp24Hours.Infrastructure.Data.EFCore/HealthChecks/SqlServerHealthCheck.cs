    ...
    ILogger<SqlServerHealthCheck> logger) : IHealthCheck
{
    ...
    private readonly SqlServerHealthCheckOptions _options = options ?? new SqlServerHealthCheckOptions();
    private readonly ILogger<SqlServerHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                string state = await GetDatabaseStateAsync(connection, cancellationToken);
            ...
            {
                int blockingCount = await GetBlockingSessionCountAsync(connection, cancellationToken);
            ...
            {
                int longRunningCount = await GetLongRunningQueryCountAsync(connection, cancellationToken);
    ...
    private async Task<List<object>> GetLongRunningQueryDetailsAsync(SqlConnection connection, CancellationToken cancellationToken)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            details.Add(new
            {
                ...
                QueryText = reader.IsDBNull(5) ? null : reader.GetString(5)?[..Math.Min(200, reader.GetString(5)?.Length ?? 0)]
            });
        }
...
public sealed class SqlServerHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["db", "database", "sqlserver", "ready"];
}
