    ...
    Func<string, DbConnection> connectionFactory) : IHealthCheck
{
    ...
    private readonly PostgreSqlHealthCheckOptions _options = options ?? new PostgreSqlHealthCheckOptions();
    private readonly ILogger<PostgreSqlHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            {
                long? lagBytes = await GetReplicationLagAsync(connection, cancellationToken);
            ...
            {
                long sizeBytes = await GetDatabaseSizeAsync(connection, cancellationToken);
            ...
            {
                int lockCount = await GetBlockedLocksCountAsync(connection, cancellationToken);
...
public sealed class PostgreSqlHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["db", "database", "postgresql", "ready"];
}
