...
/// </remarks>
public class SchemaValidator<TContext>(
    TContext dbContext,
    IOptions<SchemaValidationOptions> options,
    ILogger<SchemaValidator<TContext>> logger) : ISchemaValidator where TContext : DbContext
{
    private readonly TContext _dbContext = dbContext ?? throw new ArgumentNullException(nameof(dbContext));
    private readonly SchemaValidationOptions _options = options?.Value ?? new SchemaValidationOptions();
    private readonly ILogger<SchemaValidator<TContext>> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public ModelSummary GetModelSummary()
    {
        ...
        return new ModelSummary
        {
            ...
            EntityTypes = [.. entityTypes.Select(e => e.ClrType.Name)],
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (IEntityType entityType in entityTypes)
        {
            ...
            try
            {
                bool tableExists = await CheckTableExistsAsync(tableName, schema, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (IEntityType entityType in entityTypes)
        {
            ...
            foreach (IProperty property in properties)
            {
                ...
                try
                {
                    bool columnExists = await CheckColumnExistsAsync(tableName, columnName, cancellationToken);
