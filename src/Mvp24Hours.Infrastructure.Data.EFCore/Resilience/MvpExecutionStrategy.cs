...
public class MvpExecutionStrategy(
    ExecutionStrategyDependencies dependencies,
    EFCoreResilienceOptions options,
    ILogger? logger = null) : ExecutionStrategy(
        dependencies,
        ...
        TimeSpan.FromSeconds(options.MaxRetryDelaySeconds))
{
    ...
    private readonly ICollection<Type> _additionalTransientExceptionTypes = options.TransientExceptionTypes ?? [];
    ...
    protected override TimeSpan? GetNextDelay(Exception lastException)
    {
        ...
        double baseDelay = Math.Pow(2, _retryCount);
        double cappedDelay = Math.Min(baseDelay, _options.MaxRetryDelaySeconds);

        // Add jitter (±25%) to prevent thundering herd
        double jitter = cappedDelay * 0.25 * (2 * _jitterRandom.NextDouble() - 1);
    ...
    private bool IsSqlTransientError(Exception exception)
    {
        ...
        if (exceptionType.FullName == "System.Data.SqlClient.SqlException")
        {
            ...
            {
                int? number = (int?)numberProperty.GetValue(exception);
                if (number.HasValue)
                {
