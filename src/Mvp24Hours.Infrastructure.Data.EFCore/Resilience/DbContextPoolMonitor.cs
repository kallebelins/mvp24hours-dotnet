...
/// </remarks>
public class DbContextPoolMonitor(
    Microsoft.Extensions.Options.IOptions<EFCoreResilienceOptions> options,
    ILogger<DbContextPoolMonitor>? logger = null) : BackgroundService
{
    private readonly EFCoreResilienceOptions _options = options?.Value ?? new EFCoreResilienceOptions();
    private readonly ILogger<DbContextPoolMonitor>? _logger = logger;

    /// <summary>
    /// Gets the current pool statistics.
    /// </summary>
    public DbContextPoolStatistics Statistics { get; } = new();
    ...
    {
        Statistics.RecordHit(checkoutTime);
    }
    ...
    {
        PoolStatisticsSnapshot stats = Statistics.GetSnapshot();
        double hitRatio = stats.TotalRequests > 0
...
public class DbContextPoolStatistics
{
    ...
    public double AverageCheckoutTimeMs
    {
        get
        {
            long count = Interlocked.Read(ref _checkoutCount);
            if (count == 0)
            {
                return 0;
            }
