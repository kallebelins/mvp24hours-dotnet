...
public class TestHttpMessageHandler : HttpMessageHandler
{
    ...
    public IEnumerable<RecordedRequest> GetRequests(Func<RecordedRequest, bool> predicate)
    {
        lock (_lock)
        {
            return [.. _receivedRequests.Where(predicate)];
        }
    }
    ...
    /// </summary>
    public IEnumerable<RecordedRequest> GetGetRequests()
    {
        return GetRequests(r => r.Method == "GET");
    }

    /// <summary>
    /// Gets all POST requests.
    /// </summary>
    public IEnumerable<RecordedRequest> GetPostRequests()
    {
        return GetRequests(r => r.Method == "POST");
    }

    /// <summary>
    /// Gets all PUT requests.
    /// </summary>
    public IEnumerable<RecordedRequest> GetPutRequests()
    {
        return GetRequests(r => r.Method == "PUT");
    }

    /// <summary>
    /// Gets all DELETE requests.
    /// </summary>
    public IEnumerable<RecordedRequest> GetDeleteRequests()
    {
        return GetRequests(r => r.Method == "DELETE");
    }
    ...
    private static HttpResponseMessage CloneResponse(HttpResponseMessage original)
    {
        ...
        {
            string contentString = original.Content.ReadAsStringAsync().GetAwaiter().GetResult();
            string mediaType = original.Content.Headers.ContentType?.MediaType ?? "application/json";
        ...
        Func<HttpRequestMessage, Task<HttpResponseMessage>> responseFactory)
    {
        ...
        public Func<HttpRequestMessage, Task<HttpResponseMessage>> ResponseFactory { get; } = responseFactory;
    }
}
