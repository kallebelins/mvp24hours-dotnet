...
public class RetryPolicy : IHttpResiliencePolicy
{
    ...
    private AsyncRetryPolicy<HttpResponseMessage> CreatePolicy()
    {
        ...
        {
            foreach (int statusCode in _options.RetryStatusCodes)
            {
    ...
    private void OnRetry(
        ...
        Context context)
    {
        string? uri = context.ContainsKey("RequestUri") ? context["RequestUri"]?.ToString() : "unknown";
        string? method = context.ContainsKey("Method") ? context["Method"]?.ToString() : "unknown";
        ...
        {
            int statusCode = (int?)outcome.Result?.StatusCode ?? 0;
            string reason = outcome.Result?.ReasonPhrase ?? "Unknown";
