...
public class PropagationHeaderDelegatingHandler(
    IServiceProvider serviceProvider,
    ILogger<PropagationHeaderDelegatingHandler> logger,
    params string[] keys) : DelegatingHandler
{
    private readonly IServiceProvider _serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
    private readonly string[] _keys = keys ?? throw new ArgumentNullException(nameof(keys));
    private readonly ILogger<PropagationHeaderDelegatingHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
