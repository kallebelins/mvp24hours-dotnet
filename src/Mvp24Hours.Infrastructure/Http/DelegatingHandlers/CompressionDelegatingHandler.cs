...
public class CompressionDelegatingHandler(
    ILogger<CompressionDelegatingHandler> logger,
    CompressionHandlerOptions options) : DelegatingHandler
{
    private readonly ILogger<CompressionDelegatingHandler> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private readonly CompressionHandlerOptions _options = options ?? new CompressionHandlerOptions();
        ...
        CancellationToken cancellationToken)
    {
        ...
        byte[] originalContent = await request.Content.ReadAsByteArrayAsync(cancellationToken);
        ...
        byte[] compressedContent = await CompressAsync(originalContent, cancellationToken);
...
public class CompressionHandlerOptions
{
    ...
    /// </summary>
    public string[] CompressibleContentTypes { get; set; } =
    [
        "application/json",
        "application/xml",
        "text/plain",
