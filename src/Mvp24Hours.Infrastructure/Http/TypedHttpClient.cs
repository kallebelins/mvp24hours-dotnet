...
/// </remarks>
public class TypedHttpClient<TApi>(HttpClient httpClient, ILogger<TypedHttpClient<TApi>>? logger, IHttpClientSerializer? serializer) : ITypedHttpClient<TApi> where TApi : class
{
    private readonly ILogger<TypedHttpClient<TApi>>? _logger = logger;
    private readonly IHttpClientSerializer _serializer = serializer ?? new JsonHttpClientSerializer();
    ...
    {
        string? content = await SendAndReadStringAsync(HttpMethod.Get, url, null, headers, cancellationToken);
    ...
    {
        string? content = await SendAndReadStringAsync(HttpMethod.Post, url, data, headers, cancellationToken);
    ...
    {
        string? content = await SendAndReadStringAsync(HttpMethod.Put, url, data, headers, cancellationToken);
        return DeserializeResponse<TResponse>(content);
    }
    ...
    {
        string? content = await SendAndReadStringAsync(HttpMethod.Patch, url, data, headers, cancellationToken);
        return DeserializeResponse<TResponse>(content);
    }
    ...
    {
        string? content = await SendAndReadStringAsync(HttpMethod.Delete, url, null, headers, cancellationToken);
        return DeserializeResponse<T>(content);
    }
