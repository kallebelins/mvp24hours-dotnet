...
public abstract class LockHandleBase : ILockHandle
{
    ...
    /// <inheritdoc />
    public bool IsValid => !_disposed && !_released && DateTimeOffset.UtcNow < ExpiresAt;
    ...
    public async Task<bool> RenewAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            bool renewed = await RenewLockAsync(cancellationToken);
            if (renewed)
            {
                ExpiresAt = DateTimeOffset.UtcNow.Add(_options.LockDuration);
                return true;
            }
        }
        catch
    ...
    private async Task StartAutoRenewalAsync(CancellationToken cancellationToken)
    {
        try
        {
            while (!cancellationToken.IsCancellationRequested && !_disposed && !_released)
            {
                ...
                TimeSpan timeUntilExpiry = ExpiresAt - DateTimeOffset.UtcNow;
                if (timeUntilExpiry <= _options.RenewalInterval)
                {
                    bool renewed = await RenewAsync(cancellationToken);
                    if (!renewed)
                    {
