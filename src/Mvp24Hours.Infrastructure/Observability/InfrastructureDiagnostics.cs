...
public class InfrastructureDiagnostics(
    IEnumerable<ISubsystemDiagnosticsProvider> providers,
    ILogger<InfrastructureDiagnostics> logger) : IInfrastructureDiagnostics
{
    private readonly IEnumerable<ISubsystemDiagnosticsProvider> _providers = providers ?? throw new ArgumentNullException(nameof(providers));
    private readonly ILogger<InfrastructureDiagnostics> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (ISubsystemDiagnosticsProvider provider in _providers)
        {
            try
            {
                ...
                {
                    string metricKey = $"{provider.SubsystemName}.{key}";
