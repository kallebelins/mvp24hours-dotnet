...
public static class HttpClientExtensions
{
    ...
    {
        string? result = await HttpSendAsync(client, url, headers, METHOD_POST, data);
        if (!result.HasValue())
        {
            return default;
        }
    ...
    {
        string? result = await HttpSendAsync(client, url, headers, METHOD_GET, null);
        if (!result.HasValue())
        {
            return default;
        }
    ...
    {
        string? result = await HttpSendAsync(client, url, headers, METHOD_PUT, data);
        if (!result.HasValue())
        {
            return default;
        }
    ...
    {
        string? result = await HttpSendAsync(client, url, headers, METHOD_PATCH, data);
        if (!result.HasValue())
        {
            return default;
        }
    ...
    {
        string? result = await HttpSendAsync(client, url, headers, METHOD_DELETE, null);
        if (!result.HasValue())
        {
            return default;
        }
    ...
    public static HttpRequestMessage PropagateHeaderKey(this HttpRequestMessage request, IHttpContextAccessor? httpAccessor, params string[] keys)
    {
        if (httpAccessor?.HttpContext != null)
        {
            foreach (string key in keys)
            {
                ...
                string? headerValue = headers.GetHeaderValue(key);
