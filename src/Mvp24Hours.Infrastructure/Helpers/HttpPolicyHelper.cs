    ...
    public static IAsyncPolicy<HttpResponseMessage> GetCircuitBreakerPolicy(HttpStatusCode statusCode, int eventsBeforeBreaking = 5, int durationOfBreakInSeconds = 30)
    {
        return HttpPolicyExtensions
            ...
            .CircuitBreakerAsync(
                ...
                onBreak: (outcome, sleepDuration) => Task.Run(() => _logger?.LogError(outcome.Exception, "Circuit breaker opened. Requests will not flow for {Duration}. Reason: {Reason}",
                        sleepDuration, outcome.Exception?.GetType().Name ?? outcome.Result?.StatusCode.ToString() ?? "Unknown")),
                onReset: () => Task.Run(() => _logger?.LogInformation("Circuit breaker closed. Requests will flow normally.")),
                onHalfOpen: () => Task.Run(() => _logger?.LogInformation("Circuit breaker half-open. One test request will be allowed."))
            );
    }
