...
public class ParentJob(string parentJobId, ParentChildJobOptions? options = null) : IParentJob
{
    private readonly List<string> _childJobIds = [];

    /// <inheritdoc />
    public string ParentJobId { get; } = parentJobId ?? throw new ArgumentNullException(nameof(parentJobId));

    /// <inheritdoc />
    public IReadOnlyList<string> ChildJobIds => _childJobIds.AsReadOnly();

    /// <inheritdoc />
    public ParentChildJobOptions Options { get; } = options ?? new ParentChildJobOptions();
...
public class ChildJob(
    ...
    IEnumerable<string>? siblingDependencies = null) : IChildJob
{
    ...
    /// <inheritdoc />
    public string ChildJobId { get; } = childJobId ?? throw new ArgumentNullException(nameof(childJobId));

    /// <inheritdoc />
    public int? ExecutionOrder { get; } = executionOrder;

    /// <inheritdoc />
    public IReadOnlyList<string> SiblingDependencies { get; } = siblingDependencies?.ToList() ?? [];
}
