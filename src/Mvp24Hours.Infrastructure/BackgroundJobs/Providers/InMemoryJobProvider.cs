...
public class InMemoryJobProvider(
    IServiceProvider serviceProvider,
    ILogger<InMemoryJobProvider>? logger = null) : IJobScheduler
{
    private readonly IServiceProvider _serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
    private readonly ILogger<InMemoryJobProvider>? _logger = logger;
        ...
        where TArgs : class
    {
        ...
        string serializedArgs = JsonSerializer.Serialize(args);
        ...
        where TArgs : class
    {
        ...
        string continuationJobId = Guid.NewGuid().ToString();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string batchId = batch.BatchId ?? Guid.NewGuid().ToString();
        var batchInfo = new BatchInfo
        {
            BatchId = batchId,
            Name = batch.Name,
            Options = batch.Options,
            Jobs = [.. batch.Jobs.Select(j => j.JobId)],
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return Task.WhenAll(childJobIds.Select(async childId => await WaitForJobCompletionAsync(childId, TimeSpan.MaxValue, cancellationToken)));
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_parentChildJobs.TryGetValue(parentJobId, out List<string>? childJobIds))
        {
            foreach (string childId in childJobIds)
            {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            if (jobInfo.Status is JobStatus.Scheduled or JobStatus.Running)
            {
        ...
        where TArgs : class
    {
        ...
        try
        {
            ...
            try
            {
                ...
                TArgs? args = JsonSerializer.Deserialize<TArgs>(jobInfo.SerializedArgs) ?? throw new InvalidOperationException($"Failed to deserialize arguments for job {jobId}");
    ...
    private async Task WaitForJobCompletionAsync(string jobId, TimeSpan maxWaitTime, CancellationToken cancellationToken)
    {
        ...
        while (DateTimeOffset.UtcNow - startTime < maxWaitTime)
        {
            ...
            if (_jobs.TryGetValue(jobId, out JobInfo? jobInfo))
            {
                if (jobInfo.Status is JobStatus.Completed or
                    JobStatus.Failed or
                    JobStatus.Cancelled)
                {
                    return;
    ...
    private bool ShouldExecuteContinuation(JobStatus? parentStatus, ContinuationOptions options)
    {
        ...
        return parentStatus is JobStatus.Completed or JobStatus.Failed;
    }
