...
public static class InfrastructureServiceExtensions
{
        ...
        Action<IInfrastructureBuilder>? configure = null)
    {
        ...
        // Register options
        services.Configure<InfrastructureOptions>(opts => CopyOptions(opts, options));
        ...
        {
            services.AddInfrastructureObservability(opts => CopyObservabilityOptions(opts, options.Observability));
        }
        ...
        if (options.Resilience != null)
        {
            ...
            {
                services.Configure<RetryOptions>(opts => CopyRetryOptions(opts, options.Resilience.Retry));
            }

            if (options.Resilience.CircuitBreaker != null)
            {
                services.Configure<CircuitBreakerOptions>(opts => CopyCircuitBreakerOptions(opts, options.Resilience.CircuitBreaker));
            }
        }
    ...
    private static void LoadFromEnvironmentVariables(InfrastructureOptions options)
    {
        ...
        // Load basic options
        string? enableLazyInit = Environment.GetEnvironmentVariable($"{prefix}ENABLE_LAZY_INITIALIZATION");
        if (bool.TryParse(enableLazyInit, out bool lazyInit))
        {
            options.EnableLazyInitialization = lazyInit;
        }

        string? validateOnStart = Environment.GetEnvironmentVariable($"{prefix}VALIDATE_ON_START");
        if (bool.TryParse(validateOnStart, out bool validate))
        {
    ...
    private static void RegisterSecurityServices(IServiceCollection services, SecurityOptions securityOptions)
    {
        ...
        if (securityOptions.SecretProvider != null)
        {
            ...
            {
                services.AddAzureKeyVaultSecretProvider(opts => CopyAzureKeyVaultOptions(opts, securityOptions.AzureKeyVault));
            }
            else if (securityOptions.AwsSecretsManager != null)
            {
                services.AddAwsSecretsManagerProvider(opts => CopyAwsSecretsManagerOptions(opts, securityOptions.AwsSecretsManager));
            }
            else if (securityOptions.EnvironmentVariable != null)
            {
                services.AddEnvironmentVariableSecretProvider(opts => CopyEnvironmentVariableOptions(opts, securityOptions.EnvironmentVariable));
            }
        }
    }
        ...
        InfrastructureBuilder builder)
    {
        ...
        if (options.Email != null)
        {
            ...
            services.AddEmailService(opts => CopyEmailOptions(opts, emailOptions));
        }
        ...
        if (options.Sms != null)
        {
            ...
            services.AddSmsService(opts => CopySmsOptions(opts, smsOptions));
        }
        ...
        if (options.FileStorage != null)
        {
            ...
            services.AddFileStorage(opts => CopyFileStorageOptions(opts, fileStorageOptions));
        }
    }
    ...
    private static void CopyFileStorageOptions(FileStorageOptions target, FileStorageOptions source)
    {
        ...
        {
            target.AllowedExtensions = [.. source.AllowedExtensions];
        }
    }
}
