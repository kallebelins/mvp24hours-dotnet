...
public class AzureCommunicationSmsProvider : BaseSmsProvider
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            string url = $"{_endpoint}/sms?api-version=2021-03-07";

            // Create authorization header (HMAC-SHA256 signature)
            string dateHeader = DateTimeOffset.UtcNow.ToString("r");
            string signature = GenerateSignature("POST", url, dateHeader, _accessKey);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (valueArray.GetArrayLength() > 0)
                {
                    ...
                    string? messageId = firstResult.TryGetProperty("messageId", out JsonElement messageIdElement)
                        ? messageIdElement.GetString()
                        : null;
                    int httpStatusCode = firstResult.TryGetProperty("httpStatusCode", out JsonElement statusElement)
                    ...
                    {
                        string? errorMessage = firstResult.TryGetProperty("errorMessage", out JsonElement errorElement)
