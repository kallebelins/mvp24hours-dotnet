...
public class TwilioSmsProvider : BaseSmsProvider
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            string url = $"{_apiBaseUrl}/2010-04-01/Accounts/{_twilioOptions.AccountSid}/Messages.json";

            // Create Basic Auth header (AccountSid:AuthToken base64 encoded)
            byte[] authBytes = Encoding.UTF8.GetBytes($"{_twilioOptions.AccountSid}:{_twilioOptions.AuthToken}");
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                string? messageId = responseJson.RootElement.GetProperty("sid").GetString();
                string? status = responseJson.RootElement.GetProperty("status").GetString();
