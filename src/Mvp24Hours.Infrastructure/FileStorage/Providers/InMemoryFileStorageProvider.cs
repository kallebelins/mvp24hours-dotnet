...
public class InMemoryFileStorageProvider(
    ...
    IFileValidator? validator = null) : IFileStorage
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        lock (_lock)
        {
            ...
            byte[] content = (byte[])fileEntry.Content.Clone();
        ...
        lock (_lock)
        {
            ...
            return Task.Run(async () =>
            {
                try
                {
                    ...
                    return FileDownloadResult.Successful([], metadata);
                }
                catch (Exception ex)
                {
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        lock (_lock)
        {
            matchingFiles = [.. _files.Where(kvp => kvp.Key.StartsWith(directoryPath, StringComparison.OrdinalIgnoreCase))];
        }
        ...
        foreach (KeyValuePair<string, FileEntry> kvp in matchingFiles)
        {
            ...
            if (!recursive)
            {
                string relativePath = filePath[directoryPath.Length..];
    ...
    private string NormalizePath(string path)
    {
        ...
        {
            string basePath = _options.BasePath.Trim('/');
