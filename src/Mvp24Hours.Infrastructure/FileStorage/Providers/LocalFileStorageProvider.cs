...
public class LocalFileStorageProvider : IFileStorage
{
    ...
    public async IAsyncEnumerable<IFileMetadata> ListFilesAsync(
        ...
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        string fullPath = GetFullPath(directoryPath ?? string.Empty);
        ...
        foreach (string file in files)
        {
            ...
            try
            {
                string relativePath = Path.GetRelativePath(_baseDirectory, file).Replace('\\', '/');
    ...
    private string GetFullPath(string filePath)
    {
        ...
        string baseDirFull = Path.GetFullPath(_baseDirectory);
