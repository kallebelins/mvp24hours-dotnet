...
public class DistributedLockHealthCheck(
    IDistributedLockFactory lockFactory,
    DistributedLockHealthCheckOptions? options,
    ILogger<DistributedLockHealthCheck> logger) : IHealthCheck
{
    private readonly IDistributedLockFactory _lockFactory = lockFactory ?? throw new ArgumentNullException(nameof(lockFactory));
    private readonly DistributedLockHealthCheckOptions _options = options ?? new DistributedLockHealthCheckOptions();
    private readonly ILogger<DistributedLockHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            string testResourceName = $"health-check-{Guid.NewGuid():N}";
...
public sealed class DistributedLockHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["distributed-lock", "locking", "ready"];
}
