...
public class HttpClientHealthCheck<TApi>(
    ITypedHttpClient<TApi> httpClient,
    HttpClientHealthCheckOptions? options,
    ILogger<HttpClientHealthCheck<TApi>> logger) : IHealthCheck
    where TApi : class
{
    private readonly ITypedHttpClient<TApi> _httpClient = httpClient ?? throw new ArgumentNullException(nameof(httpClient));
    private readonly HttpClientHealthCheckOptions _options = options ?? new HttpClientHealthCheckOptions();
    private readonly ILogger<HttpClientHealthCheck<TApi>> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            string healthUrl = _options.HealthEndpoint;
...
public sealed class HttpClientHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["http", "httpclient", "ready"];
}
