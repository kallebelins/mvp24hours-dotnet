...
public class SmsServiceHealthCheck(
    ISmsService smsService,
    SmsServiceHealthCheckOptions? options,
    ILogger<SmsServiceHealthCheck> logger) : IHealthCheck
{
    private readonly ISmsService _smsService = smsService ?? throw new ArgumentNullException(nameof(smsService));
    private readonly SmsServiceHealthCheckOptions _options = options ?? new SmsServiceHealthCheckOptions();
    private readonly ILogger<SmsServiceHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
...
public sealed class SmsServiceHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["sms", "sms-service", "ready"];
}
