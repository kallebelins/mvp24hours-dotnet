...
public class FileStorageHealthCheck(
    ...
    ILogger<FileStorageHealthCheck> logger) : IHealthCheck
{
    private readonly IFileStorage _fileStorage = fileStorage ?? throw new ArgumentNullException(nameof(fileStorage));
    private readonly FileStorageHealthCheckOptions _options = options ?? new FileStorageHealthCheckOptions();
    private readonly ILogger<FileStorageHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string testFilePath = _options.TestFilePath ?? $"health-check/{Guid.NewGuid():N}.txt";
        byte[] testContent = Encoding.UTF8.GetBytes(_options.TestContent);
        ...
        try
        {
            ...
            bool exists = await _fileStorage.ExistsAsync(testFilePath, cts.Token);
            ...
            bool deleted = await _fileStorage.DeleteAsync(testFilePath, cts.Token);
...
public sealed class FileStorageHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["file-storage", "storage", "ready"];
}
