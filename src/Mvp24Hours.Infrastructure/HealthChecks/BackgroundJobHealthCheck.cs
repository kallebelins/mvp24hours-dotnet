...
public class BackgroundJobHealthCheck(
    IJobScheduler jobScheduler,
    BackgroundJobHealthCheckOptions? options,
    ILogger<BackgroundJobHealthCheck> logger) : IHealthCheck
{
    private readonly IJobScheduler _jobScheduler = jobScheduler ?? throw new ArgumentNullException(nameof(jobScheduler));
    private readonly BackgroundJobHealthCheckOptions _options = options ?? new BackgroundJobHealthCheckOptions();
    private readonly ILogger<BackgroundJobHealthCheck> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
...
public sealed class BackgroundJobHealthCheckOptions
{
    ...
    /// </summary>
    public IEnumerable<string> Tags { get; set; } = ["background-jobs", "jobs", "scheduler", "ready"];
}
