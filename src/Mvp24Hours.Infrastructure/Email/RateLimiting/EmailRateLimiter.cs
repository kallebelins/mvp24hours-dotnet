...
public class EmailRateLimiter(RateLimitOptions options)
{
    ...
    private DateTimeOffset _lastRefillTime = DateTimeOffset.UtcNow;
    private int _tokens = options.MaxRequestsPerWindow;
    ...
    public bool TryAcquire()
    {
        ...
        try
        {
            ...
            {
                RateLimitStrategy.FixedWindow => TryAcquireFixedWindow(now),
                RateLimitStrategy.SlidingWindow => TryAcquireSlidingWindow(now),
                RateLimitStrategy.TokenBucket => TryAcquireTokenBucket(now),
                _ => false,
            };
        }
        finally
    ...
    private async Task WaitForTokenBucketAsync(DateTimeOffset now, CancellationToken cancellationToken)
    {
        ...
        while (_tokens <= 0)
        {
            double refillInterval = _options.WindowSize.TotalMilliseconds / _options.MaxRequestsPerWindow;
    ...
    private void RefillTokens(DateTimeOffset now)
    {
        ...
        int tokensToAdd = (int)(elapsed.TotalMilliseconds / (_options.WindowSize.TotalMilliseconds / _options.MaxRequestsPerWindow));
