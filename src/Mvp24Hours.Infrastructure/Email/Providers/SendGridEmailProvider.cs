...
public class SendGridEmailProvider : BaseEmailProvider
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            object requestBody = CreateSendGridRequest(message);
            ...
            else
            {
                ...
                string errorMessage = $"SendGrid API error: {response.StatusCode} - {errorContent}";
    ...
    {
        object from = ParseEmailAddress(message.From ?? _sendGridOptions.DefaultFrom ?? Options.DefaultFrom ?? string.Empty);
    ...
    private object ParseEmailAddress(string address)
    {
        ...
        string? defaultName = _sendGridOptions.DefaultFromName;
