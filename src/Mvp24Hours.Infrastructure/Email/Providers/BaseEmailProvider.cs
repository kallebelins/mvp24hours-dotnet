...
public abstract class BaseEmailProvider(EmailOptions options) : IEmailService
{
    /// <summary>
    /// Gets the email options.
    /// </summary>
    protected EmailOptions Options { get; } = options ?? throw new ArgumentNullException(nameof(options));
    ...
    protected virtual IList<string> ValidateMessage(EmailMessage message)
    {
        ...
        {
            int totalRecipients = (message.To?.Count ?? 0) +
                                 (message.Cc?.Count ?? 0) +
                                 (message.Bcc?.Count ?? 0);
