...
public class AzureCommunicationEmailProvider : BaseEmailProvider
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            string requestUri = $"{_endpoint}/emails:send?api-version=2023-03-31";
            httpClient.BaseAddress = new Uri(_endpoint);

            object requestBody = CreateAzureCommunicationRequest(message);
            ...
            string signature = GenerateHmacSignature("POST", requestUri, dateHeader, jsonContent, _accessKey);
            ...
            else
            {
                ...
                string errorMessage = $"Azure Communication Services API error: {response.StatusCode} - {errorContent}";
    ...
    private EmailAddressInfo ParseEmailAddress(string address)
    {
        ...
        if (address.Contains('<') && address.Contains('>'))
        {
            ...
            string displayName = address[..startIndex].Trim().Trim('"');
            string emailAddress = address.Substring(startIndex + 1, endIndex - startIndex - 1).Trim();

            return new EmailAddressInfo { email = emailAddress, displayName = displayName };
        }

        // Simple email address
        string? defaultName = _azureOptions.DefaultFromName;
    ...
    private object CreateAzureCommunicationAttachment(Contract.IEmailAttachment attachment)
    {
        ...
        byte[]? contentBytes = attachment.Content;
        if (contentBytes == null)
        {
            using Stream? stream = attachment.GetContentStream() ?? throw new InvalidOperationException($"Cannot get content for attachment '{attachment.FileName}'.");
