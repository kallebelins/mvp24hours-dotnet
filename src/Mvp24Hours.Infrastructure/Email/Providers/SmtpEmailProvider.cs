...
public class SmtpEmailProvider : BaseEmailProvider
{
    ...
    private MailMessage CreateMailMessage(EmailMessage message)
    {
        ...
        // Set To recipients
        foreach (string to in message.To ?? Enumerable.Empty<string>())
        {
        ...
        // Set CC recipients
        foreach (string cc in message.Cc ?? Enumerable.Empty<string>())
        {
        ...
        // Set BCC recipients
        foreach (string bcc in message.Bcc ?? Enumerable.Empty<string>())
        {
    ...
    private Attachment CreateAttachment(Contract.IEmailAttachment attachment)
    {
        ...
        try
        {
            contentStream = attachment.GetContentStream() ?? throw new InvalidOperationException($"Cannot get content stream for attachment '{attachment.FileName}'.");
