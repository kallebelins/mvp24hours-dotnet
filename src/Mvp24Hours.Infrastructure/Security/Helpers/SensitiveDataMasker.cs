...
public static class SensitiveDataMasker
{
    ...
    public static string MaskPassword(string? password, int visibleChars = 0)
    {
        ...
        int maskedLength = password.Length - visibleChars;
        return new string(DefaultMaskChar, maskedLength) + password[^visibleChars..];
    }
    ...
    public static string MaskApiKey(string? apiKey, int prefixLength = 7)
    {
        ...
        return apiKey[..prefixLength] + new string(DefaultMaskChar, apiKey.Length - prefixLength);
    }
    ...
    public static string MaskCreditCard(string? cardNumber, int visibleDigits = DefaultMaskLength)
    {
        ...
        return new string(DefaultMaskChar, maskedLength) + cleaned[^visibleDigits..];
    }
    ...
    public static string MaskPhoneNumber(string? phoneNumber, int visibleDigits = DefaultMaskLength)
    {
        ...
        int maskedLength = digits.Length - visibleDigits;
        string masked = new string(DefaultMaskChar, maskedLength) + digits[^visibleDigits..];
    ...
    public static string MaskJson(string? json, IEnumerable<string> sensitiveKeys)
    {
        ...
        foreach (string key in sensitiveKeySet)
        {
            ...
            result = Regex.Replace(result, pattern, match =>
            {
                ...
                string masked = MaskPassword(value);
                return $@"""{key}"": ""{masked}""";
