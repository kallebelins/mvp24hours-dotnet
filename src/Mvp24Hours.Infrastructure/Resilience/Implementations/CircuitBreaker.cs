...
public class CircuitBreaker<TResult>(
    ...
    ILogger<CircuitBreaker<TResult>>? logger = null) : ICircuitBreaker<TResult>
{
    private readonly CircuitBreakerOptions _options = options ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<CircuitBreaker<TResult>>? _logger = logger;
    private readonly string _operationName = operationName ?? "Operation";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            string reason = $"Circuit breaker is open for {_operationName}. " +
    ...
    private void UpdateState()
    {
        ...
        if (_state is CircuitBreakerState.Closed or CircuitBreakerState.HalfOpen)
        {
            int totalOperations = _recentOperations.Count;
            if (totalOperations >= _options.MinimumThroughput)
            {
                ...
                double failureRatio = (double)failures / totalOperations;
...
[Obsolete("Deprecated: Use NativeResiliencePipeline with Microsoft.Extensions.Resilience instead. " +
          "This class will be removed in a future version. " +
          "See docs/en-us/modernization/generic-resilience.md for migration guide.", false)]
#pragma warning disable CS9113 // logger kept for API compatibility; inner breaker uses null logger
public class CircuitBreaker(
    CircuitBreakerOptions options,
    string operationName = "Operation",
    ILogger<CircuitBreaker>? logger = null) : ICircuitBreaker
#pragma warning restore CS9113
{
#pragma warning disable CS0618 // Obsolete type retained as thin wrapper over CircuitBreaker<TResult>
    private readonly CircuitBreaker<object?> _inner = new(options, operationName, null);
