...
public static class BusinessResult
{
    ...
    {
        string errorKey = exception.GetType().Name;
    ...
    public static IBusinessResult<bool> Combine(params object[] results)
    {
        ...
        foreach (object result in results)
        {
            ...
            if (resultType.IsGenericType)
            {
                ...
                {
                    if (messagesProperty.GetValue(result) is IReadOnlyCollection<IMessageResult> messages)
                    {
                ...
                {
                    bool resultHasErrors = (bool)(hasErrorsProperty.GetValue(result) ?? false);
                    if (resultHasErrors)
                    {
                        hasErrors = true;
                    }
