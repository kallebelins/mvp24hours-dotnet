...
public sealed class PhoneNumber : BaseVO, IEquatable<PhoneNumber>, IComparable<PhoneNumber>
{
    ...
    public static PhoneNumber Create(string countryCode, string areaCode, string number)
    {
        ...
        string cleanAreaCode = OnlyDigitsRegex.Replace(areaCode, "");
        string cleanNumber = OnlyDigitsRegex.Replace(number, "");
        ...
        if (cleanNumber.Length is < 6 or > 11)
        {
    ...
    public string ToFormattedString()
    {
        ...
        if (OnlyDigitsRegex.Replace(CountryCode, "") == "55" && Number.Length >= 8)
        {
            if (Number.Length == 9)
            {
                return $"{CountryCode} ({AreaCode}) {Number[..5]}-{Number[5..]}";
            }
            return $"{CountryCode} ({AreaCode}) {Number[..4]}-{Number[4..]}";
        }
    ...
    {
        return phone?.FullNumber ?? string.Empty;
    }
}
