...
public class AesEncryptionProvider : IExtendedEncryptionProvider, IDisposable
{
    ...
    public string ComputeBlindIndex(string data)
    {
        ...
        byte[] salt = _blindIndexSalt ?? _key;

        using var hmac = new HMACSHA256(salt);
        byte[] dataBytes = Encoding.UTF8.GetBytes(data.ToLowerInvariant()); // Case-insensitive
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposed)
        {
            if (disposing)
            {
                ...
                if (_blindIndexSalt != null)
                {
                    Array.Clear(_blindIndexSalt, 0, _blindIndexSalt.Length);
                }
            }
            _disposed = true;
        }
