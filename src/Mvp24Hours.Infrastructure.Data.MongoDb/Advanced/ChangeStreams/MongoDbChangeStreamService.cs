...
public class MongoDbChangeStreamService<TDocument>(
    IMongoCollection<TDocument> collection,
    ILogger<MongoDbChangeStreamService<TDocument>>? logger = null) : IMongoDbChangeStreamService<TDocument>
{
    private readonly IMongoCollection<TDocument> _collection = collection ?? throw new ArgumentNullException(nameof(collection));
    private readonly ILogger<MongoDbChangeStreamService<TDocument>>? _logger = logger;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await WatchCollectionAsync(async change => await handler(change.DocumentKey["_id"]), pipeline, null, cancellationToken);
    }
