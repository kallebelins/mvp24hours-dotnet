...
public class MongoDbTimeSeriesService<TDocument>(
    IMongoDatabase database,
    string collectionName,
    string timeField,
    string? metaField = null,
    ILogger<MongoDbTimeSeriesService<TDocument>>? logger = null) : IMongoDbTimeSeriesService<TDocument>
{
    ...
    private readonly string _timeField = timeField ?? throw new ArgumentNullException(nameof(timeField));
    private readonly string? _metaField = metaField;
    private readonly ILogger<MongoDbTimeSeriesService<TDocument>>? _logger = logger;
    ...
    public async Task<IList<TimeWindowAggregation>> AggregateByTimeWindowAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        long windowSizeMs = (long)windowSize.TotalMilliseconds;
        ...
        List<BsonDocument> results = await Collection.Aggregate<BsonDocument>(pipeline, cancellationToken: cancellationToken)
            .ToListAsync(cancellationToken);
