...
public class MongoDbShardingService(
    IMongoClient client,
    ILogger<MongoDbShardingService>? logger = null) : IMongoDbShardingService
{
    private readonly IMongoClient _client = client ?? throw new ArgumentNullException(nameof(client));
    private readonly ILogger<MongoDbShardingService>? _logger = logger;
    ...
    public async Task<IList<ShardInfo>> GetShardsAsync(CancellationToken cancellationToken = default)
    {
        ...
        return [.. shards.Select(s => new ShardInfo
        {
            ...
            Tags = s.Contains("tags") ? [.. s["tags"].AsBsonArray.Select(t => t.AsString)] : []
        })];
    }
