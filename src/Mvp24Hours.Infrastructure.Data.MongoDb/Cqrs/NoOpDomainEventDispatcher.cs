...
public class NoOpDomainEventDispatcher(ILogger<NoOpDomainEventDispatcher>? logger = null) : IDomainEventDispatcherMongoDb
{
    private readonly ILogger<NoOpDomainEventDispatcher>? _logger = logger;
    ...
    public Task DispatchEventsAsync(IHasDomainEvents entity, CancellationToken cancellationToken = default)
    {
        ...
        int eventCount = entity.DomainEvents.Count;
        if (eventCount > 0)
        {
            _logger?.LogDebug(
    ...
    public Task DispatchEventsAsync(IEnumerable<IHasDomainEvents> entities, CancellationToken cancellationToken = default)
    {
        ...
        int totalEvents = entitiesList.Sum(e => e.DomainEvents.Count);
