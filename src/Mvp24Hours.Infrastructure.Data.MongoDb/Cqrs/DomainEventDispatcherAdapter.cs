...
/// This adapter enables the MongoDB module to dispatch domain events
/// using any external dispatcher implementation through a delegate.
/// </para>
/// <para>
/// <strong>Integration with CQRS:</strong>
/// Use the extension methods in <c>MongoDbCqrsIntegrationExtensions</c> to
...
public class DomainEventDispatcherAdapter(
    Func<IEnumerable<IDomainEvent>, CancellationToken, Task> dispatchFunc,
    ILogger<DomainEventDispatcherAdapter>? logger = null) : IDomainEventDispatcherMongoDb
{
    private readonly Func<IEnumerable<IDomainEvent>, CancellationToken, Task> _dispatchFunc = dispatchFunc ?? throw new ArgumentNullException(nameof(dispatchFunc));
    private readonly ILogger<DomainEventDispatcherAdapter>? _logger = logger;
