...
public class TenantInterceptor(
    ...
    ILogger<TenantInterceptor>? logger = null) : MongoDbInterceptorBase
{
    private readonly ITenantProvider? _tenantProvider = tenantProvider ?? throw new ArgumentNullException(nameof(tenantProvider));
    private readonly bool _validateOnUpdate = validateOnUpdate;
    private readonly bool _validateOnDelete = validateOnDelete;
    private readonly bool _throwOnMissingTenant = throwOnMissingTenant;
    private readonly ILogger<TenantInterceptor>? _logger = logger;
    ...
    public override Task OnBeforeInsertAsync<T>(T entity, CancellationToken cancellationToken = default)
    {
        ...
        {
            string? currentTenantId = _tenantProvider?.TenantId;
