...
public class MongoDbInterceptorPipeline(
    IEnumerable<IMongoDbInterceptor> interceptors,
    ILogger<MongoDbInterceptorPipeline>? logger = null) : IMongoDbInterceptorPipeline
{
    private readonly IReadOnlyList<IMongoDbInterceptor> _interceptors = (interceptors ?? [])
            .OrderBy(i => i.Order)
            .ToList()
            .AsReadOnly();
    private readonly ILogger<MongoDbInterceptorPipeline>? _logger = logger;
