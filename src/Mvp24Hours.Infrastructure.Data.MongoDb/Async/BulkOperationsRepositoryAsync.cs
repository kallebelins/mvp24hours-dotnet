...
public class BulkOperationsRepositoryAsync<T>(
    Mvp24HoursContext dbContext,
    IOptions<MongoDbRepositoryOptions> options,
    ILogger<BulkOperationsRepositoryAsync<T>>? logger = null) : RepositoryAsync<T>(dbContext, options, logger), IBulkOperationsMongoDbAsync<T>
    where T : class, IEntityBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (MongoBulkWriteException<T> ex)
        {
            ...
            long insertedCount = ex.Result?.InsertedCount ?? 0;
            string errorMessage = $"Bulk insert partially failed. Inserted: {insertedCount}, Errors: {ex.WriteErrors?.Count ?? 0}. {ex.Message}";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (MongoBulkWriteException<T> ex)
        {
            ...
            long modifiedCount = ex.Result?.ModifiedCount ?? 0;
            string errorMessage = $"Bulk update partially failed. Modified: {modifiedCount}, Errors: {ex.WriteErrors?.Count ?? 0}. {ex.Message}";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        catch (MongoBulkWriteException<T> ex)
        {
            ...
            string errorMessage = $"Bulk delete partially failed. Deleted: {deletedCount}, Errors: {ex.WriteErrors?.Count ?? 0}. {ex.Message}";
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (SetPropertyCall setter in setters)
            {
                ...
                string propertyName = GetPropertyName(setter.Property);
