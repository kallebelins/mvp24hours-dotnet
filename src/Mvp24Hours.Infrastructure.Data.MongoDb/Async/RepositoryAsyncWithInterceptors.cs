...
public class RepositoryAsyncWithInterceptors<T>(
    Mvp24HoursContext dbContext,
    IOptions<MongoDbRepositoryOptions> options,
    IMongoDbInterceptorPipeline? interceptorPipeline = null,
    ILogger<RepositoryBase<T>>? logger = null) : RepositoryBase<T>(dbContext, options, logger), IRepositoryAsync<T>
    where T : class, IEntityBase
{
    private readonly IMongoDbInterceptorPipeline _interceptorPipeline = interceptorPipeline ?? NoOpInterceptorPipeline.Instance;
    ...
    public async Task AddAsync(T entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _interceptorPipeline.ExecuteInsertAsync(entity, async () => await dbEntities.InsertOneAsync(entity, cancellationToken: cancellationToken), cancellationToken);
        }
    ...
    public async Task RemoveAsync(T entity, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            bool wasSoftDeleted = await _interceptorPipeline.ExecuteDeleteAsync(
                entity,
                hardDeleteOperation: async () => await dbEntities.DeleteOneAsync(GetKeyFilter(entity), cancellationToken: cancellationToken),
