...
public class MongoDbQueryProfiler<T>(IMongoCollection<T> collection, ILogger<MongoDbQueryProfiler<T>>? logger = null)
{
    private readonly IMongoCollection<T> _collection = collection ?? throw new ArgumentNullException(nameof(collection));
    private readonly ILogger<MongoDbQueryProfiler<T>>? _logger = logger;
    ...
    public async Task<List<IndexInfo>> GetIndexesAsync(CancellationToken cancellationToken = default)
    {
        ...
        await cursor.ForEachAsync(doc => indexes.Add(new IndexInfo
        {
...
public class QueryExplainResult
{
    ...
    /// </summary>
    public BsonDocument RawOutput { get; set; } = []; // set by ParseExplainResult
