...
public class MongoDbAsyncStreaming<T>(IMongoCollection<T> collection, ILogger<MongoDbAsyncStreaming<T>>? logger = null)
{
    private readonly IMongoCollection<T> _collection = collection ?? throw new ArgumentNullException(nameof(collection));
    private readonly ILogger<MongoDbAsyncStreaming<T>>? _logger = logger;
