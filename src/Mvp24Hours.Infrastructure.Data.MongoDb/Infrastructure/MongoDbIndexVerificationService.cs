...
public sealed class MongoDbIndexVerificationService(
    IServiceProvider serviceProvider,
    IOptions<MongoDbIndexVerificationOptions> options,
    ILogger<MongoDbIndexVerificationService>? logger = null) : BackgroundService
{
    private readonly IServiceProvider _serviceProvider = serviceProvider ?? throw new ArgumentNullException(nameof(serviceProvider));
    private readonly MongoDbIndexVerificationOptions _options = options?.Value ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<MongoDbIndexVerificationService>? _logger = logger;
    ...
    private async Task VerifyIndexesAsync(CancellationToken cancellationToken)
    {
        ...
        Assembly[] assemblies = _options.AssembliesToScan ?? [];
        ...
        // Log summary
        int totalExpected = verificationResults.Sum(r => r.ExpectedIndexCount);
        int totalExisting = verificationResults.Sum(r => r.ExistingIndexCount);
        int totalCreated = verificationResults.Sum(r => r.CreatedIndexCount);
        int totalMissing = verificationResults.Sum(r => r.MissingIndexCount);
        bool anyFailed = verificationResults.Any(r => !r.Success);
        ...
        {
            string message = $"MongoDB has {totalMissing} missing indexes. " +
    ...
    {
        bool hasClassAttr = type.GetCustomAttributes<Performance.Attributes.MongoCompoundIndexAttribute>().Any();
        if (hasClassAttr)
        {
            return true;
        }
    ...
    private static List<string> GetExpectedIndexes(IMongoDbIndexManager indexManager, Type entityType)
    {
        ...
        object? result = method.Invoke(indexManager, null);
        if (result == null)
        {
            return [];
        }
        ...
        {
            foreach (object? item in enumerableResult)
            {
                PropertyInfo? optionsProp = item?.GetType().GetProperty("Options");
                object? options = optionsProp?.GetValue(item);
                PropertyInfo? nameProp = options?.GetType().GetProperty("Name");
                string? name = nameProp?.GetValue(options) as string;
        ...
        CancellationToken cancellationToken)
    {
        ...
        object? collection = getCollectionMethod?.Invoke(database, [collectionName, null]);

        if (collection != null)
        {
            if (method.Invoke(indexManager, [collection, cancellationToken]) is Task task)
            {
                await task;
            }
        }
