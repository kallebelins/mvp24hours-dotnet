...
public class MongoDbStructuredLogger(
    IOptions<MongoDbObservabilityOptions> options,
    ILogger<MongoDbStructuredLogger>? logger = null) : IDisposable
{
    private readonly MongoDbObservabilityOptions _options = options?.Value ?? new MongoDbObservabilityOptions();
    private readonly ILogger<MongoDbStructuredLogger>? _logger = logger;
    ...
    private void LogCommandStart(CommandLogContext context)
    {
        ...
        {
            string commandStr = TruncateString(context.Command.ToString(), _options.MaxLogMessageLength);
    ...
    private void WriteLog(StructuredLog log)
    {
        if (_logger != null)
        {
            string message = BuildLogMessage(log);
    ...
    private static string BuildLogMessage(StructuredLog log)
    {
        ...
        if (log.Properties.TryGetValue("CommandName", out object? cmd))
        {
            sb.Append(cmd);
        }

        if (log.Properties.TryGetValue("Collection", out object? col) && col != null)
        {
            sb.Append(" on ").Append(col);
        }

        if (log.EventName is "CommandSucceeded" or "CommandFailed")
        {
            sb.Append(log.EventName == "CommandSucceeded" ? " completed" : " FAILED");

            if (log.Properties.TryGetValue("DurationMs", out object? duration))
            {
            ...
            if (log.Properties.TryGetValue("DocumentsReturned", out object? returned))
            {
                counts.Add($"returned: {returned}");
            }

            if (log.Properties.TryGetValue("DocumentsAffected", out object? affected))
            {
                counts.Add($"affected: {affected}");
            }

            if (log.Properties.TryGetValue("DocumentsModified", out object? modified))
            {
            ...
            // Add error info
            if (log.Properties.TryGetValue("ErrorMessage", out object? error))
            {
                sb.Append($" - {error}");
            }
        }
