...
{
    private readonly ILogger<MongoDbSlowQueryLogger>? _logger = logger;
    ...
    private bool ShouldLogSlowQuery()
    {
        lock (_rateLimitLock)
        {
            ...
            {
                CurrentSlowQueryCount = 0;
                _rateLimitResetTime = now.AddMinutes(1);
            }

            // Check if we're under the limit
            if (CurrentSlowQueryCount >= _options.MaxSlowQueriesPerMinute)
            {
                return false;
            }

            CurrentSlowQueryCount++;
            return true;
        }
    }
    ...
    private string MaskSensitiveData(string input)
    {
        ...
        string result = input;
        foreach (string field in _options.SensitiveFields)
        {
    ...
    /// </summary>
    public int CurrentSlowQueryCount { get; private set; }
