...
public class MongoDbMetrics(IOptions<MongoDbObservabilityOptions>? options = null) : IMongoDbMetrics
{
    ...
    {
        string key = $"{commandName}:{errorType}";
    ...
    private class DurationTracker(int maxSamples)
    {
        private readonly ConcurrentQueue<double> _samples = new();
        private readonly int _maxSamples = maxSamples;
