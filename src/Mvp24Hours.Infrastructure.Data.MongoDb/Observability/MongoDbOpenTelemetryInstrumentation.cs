...
public class MongoDbOpenTelemetryInstrumentation : IDisposable
{
    ...
    public MongoDbOpenTelemetryInstrumentation(IOptions<MongoDbObservabilityOptions> options)
    {
        ...
        ActivitySource = new ActivitySource(_options.ActivitySourceName, "1.0.0");
    }
    ...
    {
        string spanName = $"MongoDB.{e.CommandName}";
        ...
        string? collectionName = ExtractCollectionName(e.Command, e.CommandName);
        ...
        {
            string? endpoint = e.ConnectionId.ServerId.EndPoint.ToString();
        ...
        {
            string statement = TruncateStatement(e.Command.ToString());
            activity.SetTag(DbStatement, statement);
        }

        // Add custom tags
        foreach (string tag in _options.AdditionalTraceTags)
        {
