...
public class MongoDbDurationTracker(IOptions<MongoDbObservabilityOptions> options) : IDisposable
{
    ...
    private void OnCommandStarted(CommandStartedEvent e)
    {
        ...
        string? collection = ExtractCollectionName(e.Command, e.CommandName);
    ...
    private class DurationBucket(int maxSamples, TimeSpan window)
    {
        private readonly int _maxSamples = maxSamples * 1000;
        ...
        public DurationStatistics? GetStatistics(string commandName)
        {
            DurationSample[] samples = [.. _samples];
            if (samples.Length == 0)
            {
                return new DurationStatistics { CommandName = commandName };
            }

            double[] durations = [.. samples.Select(s => s.DurationMs).OrderBy(d => d)];
        ...
        {
            DurationSample[] samples = [.. _samples];
            double[] durations = [.. samples.Select(s => s.DurationMs)];
        ...
        private static double CalculateStandardDeviation(double[] data)
        {
            ...
            double sumSquaredDiffs = data.Sum(d => Math.Pow(d - mean, 2));
