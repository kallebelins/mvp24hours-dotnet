...
public class MongoDbConnectionPoolMetrics(
    IOptions<MongoDbObservabilityOptions> options,
    ILogger<MongoDbConnectionPoolMetrics>? logger = null,
    IMongoDbMetrics? metrics = null) : IDisposable
{
    private readonly MongoDbObservabilityOptions _options = options?.Value ?? new MongoDbObservabilityOptions();
    private readonly ILogger<MongoDbConnectionPoolMetrics>? _logger = logger;
    private readonly IMongoDbMetrics? _metrics = metrics;
    ...
    {
        string endpoint = e.ServerId?.EndPoint?.ToString() ?? "unknown";
    ...
    {
        return [.. _serverStats.Select(kvp => CreateStats(kvp.Key, kvp.Value))];
    }
    ...
    private void CheckUtilizationAlert(string endpoint, ServerPoolStats stats)
    {
        ...
        double utilization = stats.MaxSize > 0 ? (double)stats.InUseCount / stats.MaxSize : 0;
