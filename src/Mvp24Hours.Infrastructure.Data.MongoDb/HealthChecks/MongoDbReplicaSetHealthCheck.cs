...
public sealed class MongoDbReplicaSetHealthCheck : IHealthCheck
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            string setName = replSetStatus.GetValue("set", "unknown").AsString;
            ...
            if (primaryOptimeDate.HasValue)
            {
                foreach (ReplicaSetMemberInfo? member in memberInfoList.Where(m => m.StateStr.Equals("SECONDARY", StringComparison.OrdinalIgnoreCase)))
                {
                    ...
                    {
                        double lag = (primaryOptimeDate.Value - member.OptimeDate.Value).TotalSeconds;
            ...
            if (issues.Count > 0)
            {
                string message = $"Replica set '{setName}' has issues: {string.Join(" ", issues)}";
