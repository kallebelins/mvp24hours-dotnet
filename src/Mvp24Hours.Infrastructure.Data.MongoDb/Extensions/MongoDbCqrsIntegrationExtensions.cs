...
public static class MongoDbCqrsIntegrationExtensions
{
    ...
    /// </summary>
    private class CqrsDomainEventDispatcherBridge(
        Infrastructure.Cqrs.Abstractions.IDomainEventDispatcher cqrsDispatcher,
        ILogger<MongoDbCqrsIntegrationExtensions.CqrsDomainEventDispatcherBridge>? logger = null) : IDomainEventDispatcherMongoDb
    {
        private readonly Infrastructure.Cqrs.Abstractions.IDomainEventDispatcher _cqrsDispatcher = cqrsDispatcher ?? throw new ArgumentNullException(nameof(cqrsDispatcher));
        private readonly ILogger<CqrsDomainEventDispatcherBridge>? _logger = logger;
    ...
    /// </summary>
    private class OutboxDomainEventWrapper(IDomainEvent domainEvent, string? correlationId = null) : Infrastructure.Cqrs.Abstractions.IIntegrationEvent
    {
        public Guid Id { get; } = Guid.NewGuid();
        public DateTime OccurredOn { get; } = domainEvent.OccurredAt;
        public string? CorrelationId { get; } = correlationId;
        public string EventType { get; } = domainEvent.GetType().AssemblyQualifiedName ?? domainEvent.GetType().FullName ?? "Unknown";
