...
public sealed class MongoDbResiliencyPolicy(MongoDbResiliencyOptions options, ILogger<MongoDbResiliencyPolicy>? logger = null) : IMongoDbResiliencyPolicy
{
    private readonly MongoDbResiliencyOptions _options = options ?? throw new ArgumentNullException(nameof(options));
    private readonly MongoDbCircuitBreaker _circuitBreaker = new(options);
    private readonly MongoDbRetryPolicy _retryPolicy = new(options, null);
    private readonly ILogger<MongoDbResiliencyPolicy>? _logger = logger;
