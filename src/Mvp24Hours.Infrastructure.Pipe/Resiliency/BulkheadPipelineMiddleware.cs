...
internal class BulkheadState : IDisposable
{
    ...
    public BulkheadState(BulkheadOptions options)
    {
        ...
        int totalPermits = options.MaxConcurrency + options.QueueLimit;
        ...
        CancellationToken cancellationToken)
    {
        ...
        // We're queued
        int queuePosition = Interlocked.Increment(ref _queuedCount);
...
public class BulkheadPipelineMiddleware(BulkheadOptions defaultOptions, ILogger<BulkheadPipelineMiddleware>? logger = null) : IPipelineMiddleware, IDisposable
{
    private readonly ILogger<BulkheadPipelineMiddleware>? _logger = logger;
    private readonly BulkheadOptions _defaultOptions = defaultOptions ?? throw new ArgumentNullException(nameof(defaultOptions));
    ...
    public async Task ExecuteAsync(IPipelineMessage message, Func<Task> next, CancellationToken cancellationToken = default)
    {
        ...
        string key = bulkheadOperation?.BulkheadKey ?? _defaultOptions.Key;
