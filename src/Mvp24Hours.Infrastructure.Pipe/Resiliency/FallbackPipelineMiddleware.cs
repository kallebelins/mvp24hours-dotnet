...
public class FallbackPipelineMiddleware(FallbackOptions defaultOptions, ILogger<FallbackPipelineMiddleware>? logger = null) : IPipelineMiddleware
{
    private readonly ILogger<FallbackPipelineMiddleware>? _logger = logger;
    private readonly FallbackOptions _defaultOptions = defaultOptions ?? throw new ArgumentNullException(nameof(defaultOptions));
...
/// </remarks>
public class FallbackPipelineMiddlewareSync(FallbackOptions defaultOptions, ILogger<FallbackPipelineMiddlewareSync>? logger = null) : IPipelineMiddlewareSync
{
    private readonly ILogger<FallbackPipelineMiddlewareSync>? _logger = logger;
    private readonly FallbackOptions _defaultOptions = defaultOptions ?? throw new ArgumentNullException(nameof(defaultOptions));
...
public class FallbackFailedException(Exception? originalException, Exception fallbackException) : AggregateException(
        $"Fallback operation failed. Original error: {originalException?.Message ?? "N/A"}. Fallback error: {fallbackException.Message}",
        originalException != null ? [originalException, fallbackException] : [fallbackException])
{
    /// <summary>
    ...
    /// </summary>
    public Exception FallbackException { get; } = fallbackException;
}
