...
internal class CircuitBreakerState(CircuitBreakerOptions options)
{
    ...
    public bool AllowExecution()
    {
        lock (_lock)
        {
            ...
            {
                PipelineCircuitState.Closed => true,
                PipelineCircuitState.HalfOpen => true,
                PipelineCircuitState.Open => false,
                _ => true,
            };
        }
    }
...
public class CircuitBreakerPipelineMiddleware(CircuitBreakerOptions defaultOptions, ILogger<CircuitBreakerPipelineMiddleware>? logger = null) : IPipelineMiddleware
{
    private readonly ILogger<CircuitBreakerPipelineMiddleware>? _logger = logger;
    private readonly CircuitBreakerOptions _defaultOptions = defaultOptions ?? throw new ArgumentNullException(nameof(defaultOptions));
    ...
    public async Task ExecuteAsync(IPipelineMessage message, Func<Task> next, CancellationToken cancellationToken = default)
    {
        ...
        string key = cbOperation?.CircuitBreakerKey ?? _defaultOptions.Key;
