...
public sealed class OperationActivityMiddleware(
    IPipelineContextAccessor contextAccessor,
    ILogger<OperationActivityMiddleware>? logger = null,
    OperationActivityOptions? options = null) : IPipelineMiddleware
{
    private readonly IPipelineContextAccessor _contextAccessor = contextAccessor ?? throw new ArgumentNullException(nameof(contextAccessor));
    private readonly ILogger<OperationActivityMiddleware>? _logger = logger;
    private readonly OperationActivityOptions _options = options ?? new OperationActivityOptions();
    ...
    public async Task ExecuteAsync(IPipelineMessage message, Func<Task> next, CancellationToken cancellationToken)
    {
        ...
        string operationName = GetCurrentOperationName(message);
    ...
    private string GetCurrentOperationName(IPipelineMessage message)
    {
        ...
        if (context != null)
        {
            string? operationName = context.GetMetadata<string>("__CurrentOperationName");
