      ...
      IForkJoinOperationAsync<TInput, TBranchInput, TBranchOutput, TOutput>
{
    ...
    public IOperationResult<TOutput> Execute(TInput input)
    {
        ...
        try
        {
            ...
            List<IOperationResult<TBranchOutput>> orderedResults = _options.PreserveOrder
                ? [.. branchResults.OrderBy(r => r.Index).Select(r => r.Result)]
                : [.. branchResults.Select(r => r.Result)];
    ...
    public async Task<IOperationResult<TOutput>> ExecuteAsync(TInput input, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Order results
            List<IOperationResult<TBranchOutput>> orderedResults = _options.PreserveOrder
                ? [.. Enumerable.Range(0, branchInputs.Count).Select(i => branchResults.TryGetValue(i, out IOperationResult<TBranchOutput>? r) ? r : OperationResult<TBranchOutput>.Failure("Branch did not complete"))]
                : [.. branchResults.Values];
    ...
    public ForkJoinOperation(
        ...
        : base(
            ...
            join: results => results.All(r => r.IsSuccess)
                ? OperationResult<IReadOnlyList<T>>.Success([.. results.Where(r => r.IsSuccess).Select(r => r.Value!)])
