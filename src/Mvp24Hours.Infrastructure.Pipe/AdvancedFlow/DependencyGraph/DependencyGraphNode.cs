...
public abstract class DependencyGraphNodeBase<TContext>(string id, string? name = null, int priority = 0) : IDependencyGraphNode<TContext>
{
    ...
    /// </summary>
    protected static OperationResult<object> Success(object? value = null)
    {
        return OperationResult<object>.Success(value ?? new object());
    }

    /// <summary>
    /// Creates a failed result with the given error message.
    /// </summary>
    protected static OperationResult<object> Failure(string errorMessage)
    {
    ...
    /// </summary>
    protected static OperationResult<object> Failure(Exception exception)
    {
        return OperationResult<object>.Failure(exception);
    }
    ...
    {
        if (nodeResults.TryGetValue(nodeId, out object? value) && value is T typedValue)
        {
            return typedValue;
        }
        return default;
...
/// </remarks>
public abstract class DependencyGraphNodeAsyncBase<TContext>(string id, string? name = null, int priority = 0) : DependencyGraphNodeBase<TContext>(id, name, priority)
{
