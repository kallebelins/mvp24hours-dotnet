...
public class CheckpointablePipeline<TState> where TState : class, new()
{
    ...
    public async Task<CheckpointableResult<TState>> ResumeAsync(string pipelineExecutionId, CancellationToken cancellationToken = default)
    {
        ...
        if (checkpoint.Status is not CheckpointStatus.Paused and not CheckpointStatus.Failed)
        {
            return new CheckpointableResult<TState>
            {
                IsSuccess = false,
        ...
        int resumeStepIndex = checkpoint.StepIndex + 1;
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            for (int i = startIndex; i < _steps.Count; i++)
            {
                ...
                {
                    string checkpointId = $"{executionId}-step-{i}";
                ...
                else
                {
                    ...
                    {
                        string errorCheckpointId = $"{executionId}-error-{i}";
                        await _checkpointStore.SaveCheckpointAsync(new PipelineCheckpoint
                        {
            ...
            bool isSuccess = failedStepId == null;
            ...
            {
                string finalCheckpointId = $"{executionId}-final";
                await _checkpointStore.SaveCheckpointAsync(new PipelineCheckpoint
                {
        ...
        catch (OperationCanceledException)
        {
            ...
            if (_options.Enabled)
            {
                string pausedCheckpointId = $"{executionId}-paused";
                await _checkpointStore.SaveCheckpointAsync(new PipelineCheckpoint
                {
