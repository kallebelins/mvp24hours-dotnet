...
public sealed class PrioritizedOperation<T>(T operation, int priority, string? group = null) : IPrioritizedOperation where T : class
{
    /// <summary>
    /// Gets the underlying operation.
    /// </summary>
    public T Operation { get; } = operation ?? throw new ArgumentNullException(nameof(operation));
...
/// </summary>
internal sealed class PriorityGrouping<T>(string? key, IEnumerable<T> items) : IGrouping<string?, T>
{
