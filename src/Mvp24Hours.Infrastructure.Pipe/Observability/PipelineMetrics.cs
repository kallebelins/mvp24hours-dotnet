...
public sealed class PipelineMetrics(int maxDurationSamples = 1000) : IPipelineMetrics
{
    #region [ Fields ]
    ...
    private readonly int _maxDurationSamples = maxDurationSamples;
    ...
    public void RecordPipelineEnd(string pipelineId, bool success)
    {
        ...
        double duration = context.Stopwatch.Elapsed.TotalMilliseconds;
        long memoryDelta = GC.GetTotalMemory(false) - context.StartMemory;
    ...
    public PipelineMetricsSnapshot GetSnapshot()
    {
        ...
        long totalPipeline = Interlocked.Read(ref _totalPipelineExecutions);
        long successPipeline = Interlocked.Read(ref _successfulPipelineExecutions);
        long totalOps = Interlocked.Read(ref _totalOperationsExecuted);
        long successOps = Interlocked.Read(ref _successfulOperations);
