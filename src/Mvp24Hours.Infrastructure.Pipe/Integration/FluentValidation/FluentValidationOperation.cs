...
public class FluentValidationOperation<T>(
    IEnumerable<IValidator<T>> validators,
    ILogger<FluentValidationOperation<T>>? logger = null,
    FluentValidationOptions? options = null) : ITypedOperationAsync<T, T>
{
    private readonly IEnumerable<IValidator<T>> _validators = validators ?? throw new ArgumentNullException(nameof(validators));
    private readonly ILogger<FluentValidationOperation<T>>? _logger = logger;
    private readonly FluentValidationOptions _options = options ?? new FluentValidationOptions();
...
/// <param name="options">Optional validation options.</param>
public class FluentValidationPipelineOperation<T>(
    IEnumerable<IValidator<T>> validators,
    string? tokenAlias = null,
    ILogger<FluentValidationPipelineOperation<T>>? logger = null,
    FluentValidationOptions? options = null) : OperationBaseAsync
{
    private readonly IEnumerable<IValidator<T>> _validators = validators ?? throw new ArgumentNullException(nameof(validators));
    private readonly ILogger<FluentValidationPipelineOperation<T>>? _logger = logger;
    private readonly FluentValidationOptions _options = options ?? new FluentValidationOptions();
    private readonly string _tokenAlias = tokenAlias ?? typeof(T).Name;
