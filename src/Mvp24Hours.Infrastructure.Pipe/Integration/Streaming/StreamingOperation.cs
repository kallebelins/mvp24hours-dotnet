...
public class TransformStreamingOperation<TInput, TOutput>(Func<TInput, CancellationToken, Task<TOutput>> transform) : StreamingOperationBase<TInput, TOutput>
{
    private readonly Func<TInput, CancellationToken, Task<TOutput>> _transform = transform ?? throw new ArgumentNullException(nameof(transform));
...
public class BatchStreamingOperation<T> : StreamingOperationBase<T, IReadOnlyList<T>>
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        await foreach (T? input in inputs.WithCancellation(cancellationToken))
        {
            ...
            bool shouldYield = batch.Count >= _batchSize || elapsed >= _timeout;
...
public class FlatMapStreamingOperation<TInput, TOutput>(Func<TInput, CancellationToken, IAsyncEnumerable<TOutput>> flatMap) : StreamingOperationBase<TInput, TOutput>
{
    private readonly Func<TInput, CancellationToken, IAsyncEnumerable<TOutput>> _flatMap = flatMap ?? throw new ArgumentNullException(nameof(flatMap));
