...
public abstract class TypedOperationBaseAsync<TInput, TOutput> : ITypedOperationAsync<TInput, TOutput>
{
    ...
    {
        return Task.FromResult<IOperationResult<TOutput>>(OperationResult<TOutput>.Success(value));
    }
    ...
    {
        return Task.FromResult<IOperationResult<TOutput>>(OperationResult<TOutput>.Failure(errorMessage));
    }
}
