...
public class ConditionalBranchOperation(ILogger<ConditionalBranchOperation>? logger = null) : IConditionalBranch, IOperation
{
    private readonly Dictionary<string, List<IOperation>> _branches = [];
    private readonly List<BranchCase> _cases = [];
    private readonly ILogger<ConditionalBranchOperation>? _logger = logger;
    ...
    public void Execute(IPipelineMessage input)
    {
        ...
        string branchKey = EvaluateBranch(input);

        List<IOperation>? branchToExecute;
...
/// </remarks>
/// <param name="logger">Optional logger for diagnostics.</param>
public class ConditionalBranchOperationAsync(ILogger<ConditionalBranchOperationAsync>? logger = null) : IConditionalBranchAsync, IOperationAsync
{
    private readonly Dictionary<string, List<IOperationAsync>> _branches = [];
    private readonly List<BranchCase> _cases = [];
    private readonly ILogger<ConditionalBranchOperationAsync>? _logger = logger;
    ...
    public async Task ExecuteAsync(IPipelineMessage input, CancellationToken cancellationToken)
    {
        ...
        string? branchKey = await EvaluateBranchAsync(input, cancellationToken);

        List<IOperationAsync>? branchToExecute;
