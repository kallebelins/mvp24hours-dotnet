...
public class RedisDistributedRateLimiter(
    ...
    InMemoryRateLimiter? fallbackLimiter = null) : IDistributedRateLimiter
{
    private readonly IDistributedCache _cache = cache ?? throw new ArgumentNullException(nameof(cache));
    private readonly DistributedRateLimitingOptions _options = options?.Value ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<RedisDistributedRateLimiter> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private readonly InMemoryRateLimiter? _fallbackLimiter = fallbackLimiter;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            byte[]? stateBytes = await _cache.GetAsync(fullKey, cancellationToken);
    ...
    {
        long currentCount = await GetCurrentCountAsync(key, cancellationToken);
        if (currentCount < 0)
        {
...
/// </remarks>
public class InMemoryRateLimiter(ILogger<InMemoryRateLimiter> logger) : IDistributedRateLimiter
{
    private readonly System.Collections.Concurrent.ConcurrentDictionary<string, RateLimitEntry> _entries = new();
    private readonly ILogger<InMemoryRateLimiter> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    public async Task<long> GetRemainingPermitsAsync(string key, int permitLimit, CancellationToken cancellationToken = default)
    {
        long current = await GetCurrentCountAsync(key, cancellationToken);
