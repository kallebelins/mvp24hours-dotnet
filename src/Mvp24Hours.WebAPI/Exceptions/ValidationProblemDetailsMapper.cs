...
public class ValidationProblemDetailsMapper(IOptions<MvpProblemDetailsOptions> options) : IExceptionToProblemDetailsMapper
{
    private readonly MvpProblemDetailsOptions _options = options?.Value ?? new MvpProblemDetailsOptions();
    ...
    public ProblemDetails Map(Exception exception, HttpContext context)
    {
        ...
        if (validationException.ValidationErrors != null)
        {
            ...
            {
                string key = error.Key ?? "General";
                if (!errors.ContainsKey(key))
                {
                    errors[key] = [error.Message ?? "Validation error"];
                }
                else
                {
                    string[] existingErrors = errors[key];
    ...
    private string BuildTypeUri(string type)
    {
        ...
        {
            string baseUri = _options.ProblemTypeBaseUri.TrimEnd('/');
            return $"{baseUri}/{type}";
        }
    ...
    private string? GetCorrelationId(HttpContext context)
    {
        ...
        if (context.Items.TryGetValue("CorrelationId", out object? itemValue) && itemValue is string correlationId)
        {
