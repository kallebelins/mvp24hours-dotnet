...
public static class OutputCachingExtensions
{
        ...
        Action<OutputCachingOptions>? configureOptions = null)
    {
        ...
        services.Configure<OutputCachingOptions>(opt =>
        {
            ...
            foreach (string path in mvpOptions.ExcludedPaths)
            {
        ...
        services.AddOutputCache(options =>
        {
            ...
            foreach (KeyValuePair<string, OutputCachePolicyOptions> policy in mvpOptions.Policies)
            {
                options.AddPolicy(policy.Key, builder => ConfigurePolicy(builder, policy.Value));
            }
            ...
            {
                options.AddPolicy("Default", builder => builder.Expire(mvpOptions.DefaultExpirationTimeSpan));
            }
    ...
    private static void ConfigurePolicy(OutputCachePolicyBuilder builder, OutputCachePolicyOptions policy)
    {
        ...
        foreach (string tag in policy.Tags)
        {
            builder.Tag(tag);
        }
        ...
        {
            builder.SetVaryByHeader([.. policy.VaryByHeader]);
        }
        ...
        {
            builder.SetVaryByRouteValue([.. policy.VaryByRouteValue]);
        }
...
/// </summary>
internal sealed class OutputCacheInvalidator(IOutputCacheStore cacheStore) : IOutputCacheInvalidator
{
