...
public static class NativeOpenApiApplicationBuilderExtensions
{
        ...
        NativeOpenApiOptions options)
    {
        ...
        if (options.EnableSwaggerUI)
        {
            app.UseSwaggerUI(swaggerOptions =>
            {
                ...
                string documentPath = options.DocumentRoutePattern.Replace("{documentName}", options.DocumentName);
                ...
                {
                    string versionDocPath = options.DocumentRoutePattern.Replace("{documentName}", version.DocumentName);
                    string versionTitle = version.IsDeprecated
        ...
        NativeOpenApiOptions options)
    {
        ...
        string indexRoute = options.DocumentRoutePattern.Split('/').First();
        ...
        if (options.EnableSwaggerUI)
        {
            ...
            {
                string html = GenerateSwaggerUIHtml(options);
        ...
        if (options.EnableReDoc)
        {
            ...
            app.MapGet($"/{options.ReDocRoutePrefix}", (HttpContext context) => Results.Redirect($"/{options.ReDocRoutePrefix}/index.html"))
            .ExcludeFromDescription();

            // Serve embedded ReDoc page at /redoc/index.html (uses CDN)
            app.MapGet($"/{options.ReDocRoutePrefix}/index.html", (HttpContext context) =>
            {
                string html = GenerateReDocHtml(options);
                return Results.Content(html, "text/html");
            })
            .ExcludeFromDescription();
        }
