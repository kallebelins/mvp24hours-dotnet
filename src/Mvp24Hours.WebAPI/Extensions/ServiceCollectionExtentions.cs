...
public static class ServiceCollectionExtentions
{
    ...
    public static IServiceCollection AddMvp24HoursWebEssential(this IServiceCollection services)
    {
        ...
        if (!services.Exists<IUrlHelper>())
        {
            ...
            {
                HttpContext httpContext = x.GetRequiredService<IHttpContextAccessor>().HttpContext
                    ?? throw new InvalidOperationException("HttpContext is not available.");
                ActionDescriptor actionDescriptor = httpContext.GetEndpoint()?.Metadata.GetMetadata<ActionDescriptor>()
    ...
    {
        services.Configure<MvcOptions>(options => options.ModelBinderProviders.Insert(0, new Mvp24HoursModelBinderProvider()));
    ...
    public static IServiceCollection AddMvp24HoursModelStateValidation(this IServiceCollection services)
    {
        ...
        services.AddControllers(options => options.Filters.Add<ModelStateValidationFilter>());
    ...
    public static IServiceCollection AddMvp24HoursRateLimiting(
        ...
        TimeSpan window)
    {
        return services.AddMvp24HoursRateLimiting(options => options.AddDefaultPolicy(permitLimit, window));
    }
        ...
        Action<CompressionOptions>? configureOptions = null)
    {
        ...
        services.AddResponseCompression(options =>
        {
            ...
            options.MimeTypes = [.. compressionOptions.MimeTypes];
        ...
        if (configureOptions != null)
        {
            ...
            if (compressionOptions.UseBrotli)
            {
                services.Configure<Microsoft.AspNetCore.ResponseCompression.BrotliCompressionProviderOptions>(options => options.Level = (CompressionLevel)Math.Clamp(compressionOptions.CompressionLevel, 0, 11));
            }
        }
    ...
    */
        ...
        Action<ContentNegotiation.ContentNegotiationBuilder>? configureBuilder = null)
    {
        ...
        services.TryAddSingleton<ContentNegotiation.IContentFormatterRegistry>(sp =>
        {
            ...
            foreach (Type formatterType in builder.CustomFormatterTypes)
            {
                try
                {
                    if (sp.GetService(formatterType) is ContentNegotiation.IContentFormatter formatter)
                    {
    ...
    {
        services.AddControllers(options => options.Filters.Add<Filters.ContentNegotiationResultFilter>());
