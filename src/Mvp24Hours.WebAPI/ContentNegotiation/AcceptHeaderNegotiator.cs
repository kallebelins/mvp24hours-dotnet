...
public class AcceptHeaderNegotiator
{
    ...
    public string? GetMediaTypeFromAcceptHeader(HttpRequest request)
    {
        ...
        string acceptHeaderValue = acceptHeader.ToString();
        ...
        try
        {
            ...
            {
                string mediaType = acceptedType.MediaType;
    ...
    public string? GetMediaTypeFromFormatParameter(HttpRequest request)
    {
        ...
        string format = formatValue.ToString().Trim();

        if (_options.FormatMappings.TryGetValue(format, out string? mediaType))
        {
            return mediaType;
        }
    ...
    public string? GetMediaTypeFromUrlSuffix(HttpRequest request)
    {
        ...
        string lastSegment = path.Split('/').LastOrDefault() ?? string.Empty;
        ...
        string extension = lastSegment[(dotIndex + 1)..].ToLowerInvariant();

        if (_options.FormatMappings.TryGetValue(extension, out string? mediaType))
        {
            return mediaType;
        }
    ...
    private static MediaTypeEntry? ParseMediaTypeEntry(string entry)
    {
        ...
        for (int i = 1; i < parts.Length; i++)
        {
            ...
            if (equalsIndex > 0)
            {
                string key = param[..equalsIndex].Trim();
                string value = param[(equalsIndex + 1)..].Trim().Trim('"');
                ...
                if (key.Equals("q", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (double.TryParse(value, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out double q))
                    {
    ...
    private static bool IsValidMediaType(string mediaType)
    {
        ...
        string type = mediaType[..slashIndex].Trim();
        string subtype = mediaType[(slashIndex + 1)..].Trim();
    ...
    private bool IsExcludedPath(PathString path)
    {
        ...
        foreach (string excludedPath in _options.ExcludedPaths)
        {
