...
public class CustomHeadersTransformer(params (string Name, string Description, bool Required)[] headers) : IOpenApiDocumentTransformer
{
    private readonly List<(string Name, string Description, bool Required)> _headers = [.. headers];
...
public class CommonResponsesTransformer(
    bool add401 = true,
    bool add403 = true,
    bool add500 = true,
    bool add503 = false) : IOpenApiDocumentTransformer
{
    private readonly bool _add401 = add401;
    private readonly bool _add403 = add403;
    private readonly bool _add500 = add500;
    private readonly bool _add503 = add503;
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (IOpenApiPathItem path in document.Paths.Values)
        {
            ...
            foreach (OpenApiOperation operation in path.Operations.Values)
            {
                ...
                {
                    string sunsetInfo = _sunsetDate.HasValue
...
public class TagFilterTransformer(
    IEnumerable<string>? includeTags = null,
    IEnumerable<string>? excludeTags = null) : IOpenApiDocumentTransformer
{
    private readonly HashSet<string> _includeTags = includeTags != null ? new HashSet<string>(includeTags, StringComparer.OrdinalIgnoreCase) : [];
    private readonly HashSet<string> _excludeTags = excludeTags != null ? new HashSet<string>(excludeTags, StringComparer.OrdinalIgnoreCase) : [];
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (string pathKey in pathsToRemove)
        {
        ...
        foreach (IOpenApiPathItem path in document.Paths.Values)
        {
            ...
            foreach (OpenApiOperation operation in path.Operations.Values)
            {
                foreach ((string? statusCode, IOpenApiResponse? response) in operation.Responses ?? [])
                {
                    ...
                    {
                        string schemaName = statusCode is "400" or "422"
