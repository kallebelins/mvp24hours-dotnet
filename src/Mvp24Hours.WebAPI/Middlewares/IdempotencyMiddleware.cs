    ...
    ILogger<IdempotencyMiddleware> logger)
{
    ...
    private bool ShouldProcessIdempotency(HttpContext context)
    {
        ...
        foreach (string pattern in _options.ExcludedPaths)
        {
    ...
    private bool IsIdempotencyKeyRequired(HttpContext context)
    {
        ...
        string path = context.Request.Path.Value ?? "/";
        foreach (string pattern in _options.RequiredPaths)
        {
            if (PathMatches(path, pattern))
            {
                return true;
        ...
        string key)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                string contentType = context.Response.ContentType ?? "application/json";
    ...
    private static string? SerializeHeaders(IHeaderDictionary headers)
    {
        ...
        foreach (KeyValuePair<string, StringValues> header in headers)
        {
            ...
            {
                headerDict[header.Key] = [.. header.Value.Where(v => v != null).Cast<string>()];
            }
        }
