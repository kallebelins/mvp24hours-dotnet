    ...
    ILogger<ContentNegotiationMiddleware> logger)
{
    ...
    private readonly ILogger<ContentNegotiationMiddleware> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    IContentFormatterRegistry registry)
{
    private readonly RequestDelegate _next = next ?? throw new ArgumentNullException(nameof(next));
    private readonly ContentNegotiationOptions _options = options?.Value ?? new ContentNegotiationOptions();
    private readonly ILogger<ResponseTransformMiddleware> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    private readonly IContentFormatterRegistry _registry = registry ?? throw new ArgumentNullException(nameof(registry));
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        if (context.Items.TryGetValue("ContentFormatter", out object? formatterObj) &&
            formatterObj is IContentFormatter formatter)
        {
            ...
            string normalizedContentType = NormalizeContentType(originalContentType);
            bool contentTypeMatches = false;
            foreach (string mt in formatter.SupportedMediaTypes)
            {
            ...
            if (!string.IsNullOrEmpty(originalContentType) && !contentTypeMatches)
            {
                ...
                try
                {
                    ...
                    string responseContent = await new StreamReader(newBody).ReadToEndAsync();
