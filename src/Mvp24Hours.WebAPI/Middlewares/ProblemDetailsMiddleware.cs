...
public class ProblemDetailsMiddleware
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        if (_options.IncludeCorrelationId &&
            problemDetails.Extensions.TryGetValue("correlationId", out object? correlationId) &&
            correlationId != null)
        {
        ...
        // Write the response
        string json = JsonSerializer.Serialize(problemDetails, _jsonOptions);
        await context.Response.WriteAsync(json);
    }

    private void LogException(Exception exception, HttpContext context)
    {
        int statusCode = _mapper.GetStatusCode(exception);
