    ...
    ILogger<RequestTimeoutMiddleware> logger)
{
    ...
    private readonly RequestTimeoutOptions _options = options?.Value ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<RequestTimeoutMiddleware> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
