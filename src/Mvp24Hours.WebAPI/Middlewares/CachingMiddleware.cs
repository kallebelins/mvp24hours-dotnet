    ...
    ILogger<CachingMiddleware> logger)
{
    ...
    private readonly MvpResponseCachingOptions _options = options?.Value ?? throw new ArgumentNullException(nameof(options));
    private readonly ILogger<CachingMiddleware> _logger = logger ?? throw new ArgumentNullException(nameof(logger));
    ...
    private void ApplyCacheProfile(HttpContext context, CacheProfile profile)
    {
        ...
        cacheControl.Public = profile.Location is ResponseCacheLocation.Any or ResponseCacheLocation.Proxy;
