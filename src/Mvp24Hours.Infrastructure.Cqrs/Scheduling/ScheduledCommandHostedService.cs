...
public class ScheduledCommandHostedService : BackgroundService
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            Type commandType = Type.GetType(entry.CommandType) ?? throw new InvalidOperationException($"Command type not found: {entry.CommandType}");

            object? command = JsonSerializer.Deserialize(entry.CommandPayload, commandType, _jsonOptions) ?? throw new InvalidOperationException($"Failed to deserialize command: {entry.Id}");
    ...
    private async Task MarkExpiredCommandsAsync(CancellationToken cancellationToken)
    {
        ...
        int count = await store.MarkExpiredAsync(cancellationToken);
        if (count > 0)
        {
    ...
    private async Task PurgeOldCommandsAsync(CancellationToken cancellationToken)
    {
        ...
        int count = await store.PurgeCompletedAsync(olderThan, cancellationToken);
