...
> **Status final:** Fases 1–10 concluídas. Dívida residual da Fase 10 (avisos ~948, `dotnet format` completo, gate estrito) **fechada na v2** — ver [`tasks-net10-warnings.md`](./tasks-net10-warnings.md) (2026-07-18): Release **0 erro(s) / 0 aviso(s)**, `MvpResidualWarnings` = só `NU1510`.
> **Baseline (início):** build com **0 erro(s)** e **4235 aviso(s)** (MSBuild, pt-BR). Distribuição aproximada dos avisos únicos por código:
>
...
- **Fechado na v2 (2026-07-18):** residual zerado — Release **0 erro(s) / 0 aviso(s)**; baseline [`warnings-baseline-v2.json`](./warnings-baseline-v2.json). Plano: [`tasks-net10-warnings.md`](./tasks-net10-warnings.md) (Fases 4–7).
...
- **Fechado na v2 (2026-07-18):** regras de estilo elevadas a `warning`; `dotnet format` completo (sem `--severity error`) + CI `verify-no-changes` com `--exclude-diagnostics IDE0130 IDE1006`. Tarefa 7.2 em [`tasks-net10-warnings.md`](./tasks-net10-warnings.md).
...
- **Fechado na v2 (2026-07-18):** `MvpResidualWarnings` reduzido a **somente `NU1510`**; gate estrito → **0 erro(s) / 0 aviso(s)**. Tarefa 7.1 em [`tasks-net10-warnings.md`](./tasks-net10-warnings.md).
- `.github/workflows/ci.yml` (job `code-quality`, step `🔍 Run static code analysis`), `src/Directory.Build.props`
- https://learn.microsoft.com/visualstudio/msbuild/msbuild-warnings-as-errors

[x] 10.4 - Atualizar `CHANGELOG.md` com o resumo da modernização para .NET 10 — Task [#87313](https://bancorbras-ti.visualstudio.com/Bancorbrás-Agile/_workitems/edit/87313)
...
- **Concluído (17/07/2026):** adicionada entrada **`## [10.0.0] - 2026-07 🚀 Major Release`** no topo de [`CHANGELOG.md`](../CHANGELOG.md) (acima de `9.1.210`), no padrão Keep a Changelog + SemVer já usado no arquivo. Seções: **Mudado** (TFM `net10.0` em todos os projetos, `LangVersion=latest`/C# 14, Nullable habilitado com assinaturas de nulidade ajustadas, `required` em `SetPropertyCall.Property`/`EncryptionOptions.Key`, overrides/interfaces `ReadJson`/`Enumeration`/`IValueProvider.SetValue`, CPM + `Directory.Build.props`); **Segurança** (pin NU1903 `Cryptography.Xml` 10.0.10 + GHSA-37gx/GHSA-w3x6, security-scan); **Substituído** (Fase 5: CircuitBreaker→NativeResiliencePipeline, `Microsoft.Data.SqlClient`, `X509CertificateLoader`, remoção de `ServicePointManager` com mudança de comportamento do SMTP `ServerCertificateValidationCallback`, `Rfc2898DeriveBytes.Pbkdf2`, `DefaultAWSCredentialsIdentityResolver`); **Corrigido** (4235→969 avisos, CS0168/0219/1718/0108/CA2022/xUnit1031, NU1510); **Testes** (2298/0/4 + traits Unit/Integration); **CI/CD** (SDK 10.0.x, gate escopado). Versão dos `.csproj` (`9.1.21`) não bumpada — fora do escopo desta task.
- **Atualizado na v2 (2026-07-18):** entrada `[10.0.0]` revisada — avisos **4235 → 0**, gate estrito (`NU1510` apenas), formatação completa e suíte revalidada. Tarefa 7.4 em [`tasks-net10-warnings.md`](./tasks-net10-warnings.md).
- `CHANGELOG.md`
